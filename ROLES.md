...
| `layout` | `(session, { round, window, reply, fixed, live, query, idle_s, helpers }) -> { id, budget, slots, jobs, fits, why }`; `rules` contains verified user directives, `observations` contains lower-trust metadata. Both consume the `pinned` quota. Legacy mixed `pinned` text is not verified authority. | magi sends every live entry whole, every round. An over-budget context goes to the provider and is refused there. This is the most expensive extension to lack. |
...
| `jobs` | `(session, {helpers?, retry_extraction?}) -> [job]`; `{inspect=true}` instead returns one extraction/job status object without issuing work. | No helper work runs between turns: no background summaries, no memory curation. |

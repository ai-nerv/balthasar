...
use aeon_model::{Importance, Tier};
...
impl Section {
    ...
    #[must_use]
    pub fn all(config: &aeon_lua::Config) -> Vec<Self> {
    ...
    #[must_use]
    pub fn takes(&self, memory: &aeon_model::Memory, project: bool) -> bool {
...
mod tests {
    use super::*;
    use aeon_model::{Body, Memory, MemoryId, ScopeId};
    ...
    fn a_section_with_no_tiers_takes_any() {
        ...
        assert!(section.takes(
            &memory(Tier::Episode, Body::note("x", aeon_model::NoteKind::Claim)),
            true
        ));
    }
    ...
    fn a_predicate_filter_excludes_what_has_no_slot() {
        ...
        assert!(!section.takes(
            &memory(Tier::Fact, Body::note("x", aeon_model::NoteKind::Claim)),
            true
        ));
    }
    ...
    fn sections_that_did_not_say_keep_the_order_they_were_declared_in() {
        let mut config = aeon_lua::Config::default();
        for id in ["a", "b", "c"] {
            config
                .registered

...
use aeon_model::{Memory, ScopeId, Timestamp};
use aeon_store::{Recall, Store, Weights};
...
) -> Result<Context, aeon_store::StoreError> {
...
fn gather(
    ...
    section: &Section,
    ask: &Ask,
) -> Result<Vec<Memory>, aeon_store::StoreError> {
...
pub fn stores_for(
    scope: &ScopeId,
    open: impl Fn(&ScopeId) -> Result<Store, aeon_store::StoreError>,
) -> Result<Vec<(Store, bool)>, aeon_store::StoreError> {
    if scope.is_global() {

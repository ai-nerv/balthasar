...
use aeon_model::{Memory, ScopeId, SessionId, Tier, Timestamp, Witness, WitnessId, floor};
...
fn said(
    ...
    let mut scratch = Memory::new(
        ...
        Tier::Scratch,
        scope.clone(),
        aeon_model::Body::note(text, aeon_model::NoteKind::Observation),
        at,
    );
    scratch.session = Some(id.clone());
    scratch.temporal = aeon_model::Temporal::recalled(at, at);
    ...
    store
        .observe(
            ...
            &aeon_store::Entry {
                ...
                tokens: 10,
                state: if ok {
                    aeon_store::State::Live
                } else {
                    aeon_store::State::Dropped
                },
                pinned: false,
                at,
            },

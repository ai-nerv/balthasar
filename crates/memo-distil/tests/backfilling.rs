...
use memo_distil::{Ingest, ingest};
use memo_lua::{Engine, Settings};
use memo_model::{ScopeId, Tier, Timestamp, WitnessKind};
use memo_store::{Recall, Store};
...
/// Every source adapter memo ships, loaded, with the ids they registered.
fn shipped() -> (Engine, Vec<String>) {
    let mut engine = Engine::new();
    let files: Vec<(PathBuf, bool)> =
        memo_lua::glob_paths(&format!("{}/sources/*.lua", config_dir().display()))
            .into_iter()
            .map(|p| (PathBuf::from(p), true))
            .collect();
    assert!(!files.is_empty(), "memo ships at least one source adapter");
    engine
        .read(&files)
...
fn journal(name: &str, lines: &[String]) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("memo-ingest-{name}"));
...
fn ingest_one(source: &str, file: &Path, store: &mut Store, dry_run: bool) -> memo_distil::Report {
    let (mut engine, _) = shipped();
    // Keep the adapter's own `meta` and `line`, and give it one file to walk.
    engine
        .run(
            &format!(
                "local held = __memo_specs.source[{source:?}]\n\
...
fn a_session_is_read_and_what_it_taught_is_kept() {
    ...
    let found = store
        .recall(&Recall {
            ...
            weights: memo_store::Weights::default().without_vectors(),
            embedding: None,
            near: true,
            reinforce: false,
        })

...
//! model is unambiguously better at is the one thing memo could not do. This is that, arranged
...
//! alone**. It has to be corroborated by something memo worked out for itself before it is a
...
use memo_model::{Body, Importance, NoteKind, Tier, WitnessKind};
...
fn claims(answer: &str, backend: &str) -> Vec<Candidate> {
    ...
    for line in answer.lines() {
        ...
        // The one place in memo where text a model wrote becomes a stored claim, which makes it

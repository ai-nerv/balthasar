...
pub struct Told {
    ...
    /// How it ended.
    pub outcome: memo_model::Outcome,
...
pub fn tell(turns: &[Observation]) -> Told {
    ...
    let outcome = match turns.iter().rev().find(|t| t.ok.is_some()) {
        Some(last) if last.worked() => memo_model::Outcome::Done,
        Some(_) => memo_model::Outcome::Failed,
        None => memo_model::Outcome::Open,
    };
    ...
    let mut entities: Vec<String> = turns
        .iter()
        .flat_map(|t| memo_store::entities_in(&t.text))
        .map(|e| e.display)
...
#[must_use]
pub fn avoidance(told: &Told, condition: &str) -> Option<memo_model::Avoidance> {
    ...
    if told.outcome != memo_model::Outcome::Failed {
        return None;
    }
...
mod tests {
    ...
    fn what_failed_and_was_never_fixed_is_left_open() {
        ...
        assert_eq!(held.outcome, memo_model::Outcome::Failed);
    }
    ...
    fn a_run_that_stopped_mid_flight_is_open_not_failed() {
        ...
        assert_eq!(held.outcome, memo_model::Outcome::Open);
    ...
    fn a_fixed_failure_is_not_left_open() {
        let held = tell(&a_repair());
        assert!(held.unresolved.is_empty());
        assert_eq!(held.outcome, memo_model::Outcome::Done);
    }

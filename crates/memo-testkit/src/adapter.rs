...
//! questions about it. They are useful and they are not what memo is for, so they sit here as
...
//!
//! **The local suite stays the gate.** These measure whether memo can answer questions about a
//! conversation. The thing memo is actually for — does session k+1 stop rediscovering what
//! session k learned — is not something any of them ask.

use crate::{Act, Case, Category, Expect, Probe};
use std::path::Path;

/// A benchmark memo knows how to read.
...
impl Family {
    ...
    /// Which of memo's categories its questions map onto.
    ///
    ...
    /// What it cannot tell you about, however well memo scores.
    #[must_use]
...
/// clever mapping would be a place for memo to score well by understanding the benchmark rather
/// than by remembering.
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let at = std::env::temp_dir().join(format!("memo-adapter-{name}"));

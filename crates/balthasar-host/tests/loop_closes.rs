...
//! what it ran and how it went. balthasar works out for itself whether the action followed anything
...
fn an_action_that_followed_a_memory_attributes_itself() {
    // The centre of it. The harness says what it ran; balthasar works out that the memory naming
    ...
    assert_eq!(
        ...
        balthasar_model::Attribution::Structural,
        "balthasar did not see the match for itself"
    );
...
fn an_unrelated_action_credits_nothing() {
    ...
    assert_eq!(recorded.attribution, balthasar_model::Attribution::Proximal);
}

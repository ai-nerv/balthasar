...
use balthasar_host::{Door, answer};
use balthasar_ipc::{Peer, Request};
use balthasar_model::{ScopeId, Timestamp, WitnessKind, floor};
...
impl Held {
    ...
    fn ask(&mut self, door: &Door, request: &Request) -> balthasar_ipc::Reply {
        let mut at = balthasar_host::Answering {
            store: &mut self.0,
            scrollback: None,
            scratch: None,
...
fn nothing_shaped_like_running_a_command_is_answered() {
    let mut held = Held::new();
    for name in balthasar_host::NEVER {
...
fn a_peer_may_not_forge_an_imperative_by_asking_for_one() {
    ...
    let memory = held
        .0
        .get(&balthasar_model::MemoryId::new(id))
        .expect("get")
        .expect("there");
...
fn every_write_by_a_peer_says_which_process() {
    // `balthasar why` has to be able to answer "which process believes this".
    let mut held = Held::new();
    let reply = held.ask(
        &peer(),

...
//!
//! Every claim it returns becomes a [`WitnessKind::Inferred`] candidate, weighted 0.35 — above
//! the hold floor so it waits in scratch, below the promote floor so it **cannot cross alone**.
//!
//! Only the person's own turns go into the prompt, so a fetched page, a file, or a command's
//! output cannot reach the model to steer what it proposes.
//!
//! With no backend configured or none reachable, [`propose`] answers with nothing and the
//! extractive path runs exactly as it did.
...
///
/// The most recent turns, because a correction is about what just happened.
const PROMPT_CHARS: usize = 6_000;

/// How many claims one call may return, so one call cannot fill a project's scratch.
...
fn claims(answer: &str, backend: &str) -> Vec<Candidate> {
    ...
    for line in answer.lines() {
        ...
        // The one place in balthasar where text a model wrote becomes a stored claim.

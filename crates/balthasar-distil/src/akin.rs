...
//!
//! The judgement itself is [`balthasar_model::same_claim`] — whether two claims are one claim is a
...
pub fn merge(clusters: Vec<Cluster>) -> Vec<Akin> {
    ...
    for cluster in clusters {
        let joined = out
            .iter_mut()
            .take(COMPARED)
            .find(|held| balthasar_model::same_claim(&held.cluster.text, &cluster.text));
...
mod tests {
    ...
    fn cluster(text: &str, sessions: &[&str]) -> Cluster {
        Cluster {
            text: text.to_owned(),
            hash: balthasar_model::content_hash(text),
    ...
    fn a_claim_never_corroborates_its_own_replacement() {
        ...
        for (a, b) in [
            ...
            assert!(
                !balthasar_model::same_claim(a, b),
    ...
    fn a_claim_with_nothing_in_it_matches_nothing() {
        assert!(!balthasar_model::same_claim("", ""));
        assert!(!balthasar_model::same_claim("the it is", "of and to"));
        assert_eq!(balthasar_model::claim_overlap("", "we use make"), 0.0);
    }
}

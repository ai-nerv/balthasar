...
pub struct Told {
    ...
    /// How it ended.
    pub outcome: balthasar_model::Outcome,
...
pub fn tell(turns: &[Observation]) -> Told {
    ...
    let outcome = match turns.iter().rev().find(|t| t.ok.is_some()) {
        Some(last) if last.worked() => balthasar_model::Outcome::Done,
        Some(_) => balthasar_model::Outcome::Failed,
        None => balthasar_model::Outcome::Open,
    };
    ...
    let mut entities: Vec<String> = turns
        .iter()
        .flat_map(|t| balthasar_store::entities_in(&t.text))
        .map(|e| e.display)
...
#[must_use]
pub fn avoidance(told: &Told, condition: &str) -> Option<balthasar_model::Avoidance> {
    ...
    if told.outcome != balthasar_model::Outcome::Failed {
        return None;
    }
    ...
    let held = balthasar_model::Avoidance {
        rejected,
...
mod tests {
    ...
    fn what_failed_and_was_never_fixed_is_left_open() {
        ...
        assert_eq!(held.outcome, balthasar_model::Outcome::Failed);
    }
    ...
    fn a_run_that_stopped_mid_flight_is_open_not_failed() {
        ...
        assert_eq!(held.outcome, balthasar_model::Outcome::Open);
    ...
    fn a_fixed_failure_is_not_left_open() {
        let held = tell(&a_repair());
        assert!(held.unresolved.is_empty());
        assert_eq!(held.outcome, balthasar_model::Outcome::Done);
    }

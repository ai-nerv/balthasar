...
use balthasar_model::{Memory, ScopeId, SessionId, Timestamp, Witness, WitnessId};
...
fn hold(store: &mut Store, from: &Provenance, candidate: &Candidate) -> Result<(), DistilError> {
    ...
    memory.temporal = balthasar_model::Temporal::recalled(from.now, from.happened);
...
fn witness_for(from: &Provenance, candidate: &Candidate) -> Witness {
    let witness = Witness::new(
        WitnessId::new(format!(
            ...
            &balthasar_model::content_hash(&candidate.text())[..8]
        )),
        candidate.witness,
        from.session.clone(),
        from.scope.clone(),
...
fn write(
    ...
    memory.temporal = balthasar_model::Temporal::recalled(from.now, happened);

...
use balthasar_model::{Body, Tier, Timestamp, WitnessKind};
...
impl Candidate {
    ...
    pub fn new(body: Body, tier: Tier, witness: WitnessKind, from: impl Into<String>) -> Self {
        Self {
            ...
            witness,
            cursor: None,
            importance: balthasar_model::Importance::Normal,
            from: from.into(),
            pinned: false,
        }
    }
    ...
    pub fn pinned(mut self) -> Self {
        self.pinned = true;
        self.importance = balthasar_model::Importance::Critical;
        self
    }
    ...
    #[must_use]
    pub fn fading(mut self, importance: balthasar_model::Importance) -> Self {
...
pub enum Verdict {
    ...
    Refuse {
        /// Why, so `balthasar ingest --explain` can say.
        reason: String,
    },
}

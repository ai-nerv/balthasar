...
use balthasar_distil::{Budget, Distil, DistilFailure, Observation, Role, Verdict, propose, weigh};
use balthasar_model::{SessionId, Tier, Timestamp, WitnessKind, floor};
use balthasar_store::{Store, mint};

const NOW: Timestamp = 1_756_000_000;

fn scope() -> balthasar_model::ScopeId {
...
fn a_model_reads_what_the_rules_could_not() {
    ...
    // correction, so every rule balthasar has is blind to it.
    let turns = [said("we moved off Heroku last month, it's all fly.io now")];
    assert!(
        balthasar_distil::extract(&turns, &["remember".to_owned()])
            .candidates
            .is_empty(),
...
fn an_inferred_claim_waits_in_scratch_and_is_corroborated_later() {
    ...
    let mut waiting = balthasar_model::Memory::new(
        mint(NOW),
        Tier::Scratch,
        scope(),
        balthasar_model::Body::note("we deploy with fly.io", balthasar_model::NoteKind::Claim),
        NOW,
    );
    waiting.session = Some(SessionId::new("01READ"));
    let witness = balthasar_model::Witness::new(
        balthasar_model::WitnessId::new("w-inferred"),
    ...
    let mut agreed = balthasar_model::Memory::new(
        mint(NOW),
        Tier::Scratch,
        scope(),
        balthasar_model::Body::note("we deploy with fly.io", balthasar_model::NoteKind::Claim),
    ...
    store
        .remember(
            ...
            balthasar_model::Witness::new(
                balthasar_model::WitnessId::new("w-said"),

//! The gap this closes: a run streamed live into balthasar's own scrollback is read by the rules.
//!
//! Before this, the extractive half of the ladder only ran on `balthasar ingest`, which walks a
...
use balthasar_distil::{Ingest, TRANSCRIPT_SOURCE, distil_run, undistilled};
use balthasar_lua::{Engine, Settings};
use balthasar_model::{ScopeId, SessionId, Tier, Timestamp, WitnessKind};
use balthasar_store::{Recall, Store, Transcript, Turn};
...
fn reading_a_run_twice_teaches_nothing_the_second_time() {
    // Stamped like an ingest, so `balthasar consolidate` can run on a timer without re-reading every
...
fn a_correction_nobody_marked_is_learned_from_the_contradiction() {
    ...
    // no "no,", and every word-matching rule balthasar has is blind to it — but the project is
    ...
    let live: Vec<String> = found
        .iter()
        .map(|s| &s.memory)
        .filter(|m| m.confidence >= balthasar_model::floor::INJECT)
        .map(balthasar_model::Memory::text)
        .collect();
    assert!(

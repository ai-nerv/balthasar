...
/// Where aeon binds its sockets.
///
/// `$XDG_RUNTIME_DIR/aeon`, falling back to a per-user directory in the temporary one. Named
...
pub fn socket_dir() -> PathBuf {
    if let Some(runtime) = std::env::var_os("XDG_RUNTIME_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(runtime).join("aeon");
    }
    let uid = rustix::process::getuid().as_raw();
    std::env::temp_dir().join(format!("aeon-{uid}"))
}
...
///
/// aeon's state is SQLite on disk, outside the process, which is exactly the condition the
/// family names for a spawnable tool: a fresh process knows everything the running one does.
/// The path written is aeon's own, absolute — never a name resolved through `$PATH`. Executing
...
impl Listener {
        ...
        if path.exists() {
            if UnixStream::connect(&path).is_ok() {
                return Err(std::io::Error::new(
                    ...
                    format!("an aeon is already listening on {}", path.display()),
                ));
            }
    ...
    ///
    /// One connection at a time, and each connection may carry many calls. aeon is asked
    ...
    pub fn serve(&self, mut answer: impl FnMut(&Peer, Request) -> Reply) -> std::io::Result<()> {
        for incoming in self.listener.incoming() {
            ...
            let Some(peer) = Peer::of(&stream) else {
                ...
                let _ = refuse(&mut stream, "aeon cannot identify this caller");
                continue;
            };
            if !peer.is_owner() {
...
mod tests {
    ...
    fn a_socket_lives_under_the_tools_own_name() {
        ...
        assert!(
            path.to_string_lossy().contains("/aeon/"),
            "{}",
            path.display()
        );

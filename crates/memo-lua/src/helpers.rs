...
//! memo's process, so it gets the handful of things its handlers actually need and not a
//! standard library.
...
/// Anything the config passed to `memo.log`.
pub type Logged = Rc<RefCell<Vec<String>>>;

/// Install `memo.json`, `memo.path`, `memo.git`, `memo.text`, `memo.log` and the few constants.
pub fn install<'gc>(ctx: luna::Context<'gc>, memo: Table<'gc>, logged: &Logged) {
    memo.set(ctx, "json", json_table(ctx)).ok();
    memo.set(ctx, "path", path_table(ctx)).ok();
    memo.set(ctx, "git", git_table(ctx)).ok();
    memo.set(ctx, "text", text_table(ctx)).ok();
    memo.set(ctx, "fs", fs_table(ctx)).ok();
    ...
    memo.set(ctx, "log", log).ok();

    memo.set(ctx, "looks_like_secret", secret_check(ctx)).ok();
    ...
    for (name, value) in [
        ...
        memo.set(ctx, name, text).ok();
    }
}

/// `memo.json.encode` and `memo.json.decode`, so a source adapter does not carry a parser.
fn json_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let encode = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Some(json) = convert::to_json(ctx, value, 0) else {
            return Err(convert::raise(
                ctx,
                "memo.json.encode: cannot be written down",
            ));
        };
    ...
    let decode = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.json.decode: expects a string"));
        };
...
/// `memo.path.basename` and `memo.path.join`.
fn path_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let basename = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.path.basename: expects a path"));
        };
...
/// `memo.git.common_dir`, so a scope handler can put every worktree in one memory.
fn git_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let common = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.git.common_dir: expects a path"));
        };
        let path = String::from_utf8_lossy(text.as_bytes()).into_owned();
...
/// `memo.text.tail`, for a mask handler trimming a wall of output to its last few lines.
fn text_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let tail = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.text.tail: expects a string"));
        };
...
/// `memo.fs.glob`, `memo.fs.read` and `memo.fs.ls`.
///
...
fn fs_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let glob = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.fs.glob: expects a pattern"));
        };
    ...
    let read = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "memo.fs.read: expects a path"));
        };
        let path = String::from_utf8_lossy(text.as_bytes()).into_owned();
...
/// `memo.looks_like_secret`, so the shipped gate does not need a regex engine in Lua.
...
mod tests {
    ...
    fn a_glob_matches_by_prefix_and_extension() {
        let dir = std::env::temp_dir().join("memo-glob-test");

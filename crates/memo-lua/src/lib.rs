//! memo's configuration, which is a program rather than a data file.
//!
...
//! # The two namespaces
//!
//! A config author must be able to tell a handler's return contract without checking, so memo
//! spends a namespace on it:
//!
//! | | contract | a raise |
//! |---|---|---|
//! | `memo.on.<question>` | **asked.** `nil` = not mine, carry on. A table = do this instead. First non-`nil` wins. | reported, that handler skipped, the rest still run |
//! | `memo.did.<verb>` | **told.** Pure side effect; the return value is ignored. | reported and survived |
//!

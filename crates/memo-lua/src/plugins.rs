...
pub struct Roots {
    /// `$XDG_CONFIG_HOME/memo`, the owner's own.
    pub config: Option<PathBuf>,
    /// `$XDG_DATA_HOME/memo/site`, where installed packages live.
    pub site: Option<PathBuf>,
    /// The working directory, whose `.memo.lua` may choose but not declare.
...
impl Roots {
    ...
    pub fn discovered(cwd: &Path) -> Self {
        Self {
            config: Some(PathBuf::from(crate::helpers::config_home()).join("memo")),
            site: Some(PathBuf::from(crate::helpers::data_home()).join("memo/site")),
...
///   ./.memo.lua                       may choose, may not declare
/// ```
#[must_use]
pub fn runtimepath(roots: &Roots) -> Vec<(PathBuf, bool)> {
    ...
    if let Some(project) = &roots.project {
        let local = project.join(".memo.lua");
        if local.is_file() {
...
///
/// `memo.trusted = { "/home/you/work" }` in the owner's own configuration. A directory under a
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let at = std::env::temp_dir().join(format!("memo-rtp-{name}"));

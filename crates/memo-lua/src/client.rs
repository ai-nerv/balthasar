//! The client library other programs load to talk to a running memo.
//!
//! Plain Lua, and *copied* from the family rather than written: oslo ships `client.lua`, hexe
//! ships `hexe.lua`, and this is the same file with memo's identity and memo's verbs. The
...
/// The client library, as a program receives it from `memo lua-api`.
pub const CLIENT: &str = include_str!("../lua/memo.lua");
...
mod tests {
    ...
    /// Load the client in memo's own VM and ask it something.
    fn probe(script: &str) -> String {
        let mut engine = Engine::new();
        let source = format!(
            r#"
            local chunk = assert(load({CLIENT:?}, "memo.lua"))
            local client = chunk(nil)
            memo.answer = tostring({script})
            "#
        );
        engine
    ...
    fn the_stub_loads_in_memos_own_vm() {
        ...
        assert_eq!(probe("client._NAME"), "memo");
    }
    ...
    fn the_stub_carries_its_own_json() {
        // It runs inside somebody else's VM, so it cannot reach memo's. A client that needed
    ...
    fn the_stub_holds_its_connection() {
        // memo is asked several times per turn, which is not how a control socket that gets
    ...
    fn the_stub_looks_for_every_sibling_rather_than_only_itself() {
        ...
        // business, and the stub is Lua. Naming one here would put a harness in memo's Rust,
        ...
        assert!(
            line.contains("\"memo\""),
            "memo is not in its own family list"
        );
    }

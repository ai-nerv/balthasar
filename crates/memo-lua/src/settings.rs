...
//!
//! The bridge between "the config said something" and "memo behaves differently". Every knob
...
use crate::Config;
use memo_model::{Importance, WitnessKind, floor};
...
/// Everything a configuration decided, in the shapes memo uses.
...
pub struct Settings {
    ...
    /// Directories whose own `.memo.lua` may declare.
...
impl Default for Settings {
    /// What memo does when nothing has been configured.
    ///
...
impl Settings {
    ...
    pub fn from(config: &Config) -> Self {
        ...
        Self {
            ...
            budget,
            ledger: Ledger {
                // One table, `memo.outcome`, rather than a flat flag beside a nested number.
    ...
    /// A digest of every setting that changes what memo does.
    ///
    ...
    pub fn fingerprint(&self) -> String {
        memo_model::content_hash(&format!(
...
mod tests {
    ...
    fn a_nested_knob_can_be_set_alone() {
        // `memo.decay.normal = 0.02` must not reset the other three to zero.

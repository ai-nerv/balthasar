...
/// Questions `memo.on.<name>` may be registered against.
///
...
/// which is what keeps "memo asked and used the answer" distinguishable from "memo mentioned it
...
/// Events `memo.did.<name>` may be registered against.
pub const TOLD: &[&str] = &[
    "observe",
    "promote",
    "contradict",
...
pub const ON: &str = "__memo_on";
/// Where told-handlers live inside the VM.
pub const DID: &str = "__memo_did";
/// Where a call's arguments are left for the generated chunk.
pub const ARGS: &str = "__memo_args";
/// Where a call's answer is left for Rust.
pub const ANSWER: &str = "__memo_answer";

/// Build `memo.on` and `memo.did`, and the tables behind them.
pub fn install<'gc>(ctx: luna::Context<'gc>, memo: Table<'gc>) {
    for (field, store, names) in [("on", ON, ASKED), ("did", DID, TOLD)] {
            ...
            let register = Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
                ...
                if !matches!(handler, Value::Function(_)) {
                    return Err(convert::raise(
                        ctx,
                        &format!("memo.{field}.{which}: expects a function"),
                    ));
                }
        ...
        memo.set(ctx, field, namespace).ok();
    }
}

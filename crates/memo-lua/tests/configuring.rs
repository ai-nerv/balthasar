...
use memo_lua::{Engine, LuaError, Settings};
...
fn a_setting_is_assigned_and_read_back() {
    let engine = run("memo.inject_floor = 0.6");
...
fn a_config_can_read_and_reassign_its_own_settings() {
    ...
    let engine =
        run("memo.inject_floor = 0.6\nif memo.inject_floor > 0.5 then memo.inject_floor = 0.4 end");
    assert_eq!(engine.config().number("inject_floor"), Some(0.4));
}

#[test]
fn a_nested_table_exists_before_a_config_touches_it() {
    // `memo.decay.normal = 0.02` must work without a config writing `memo.decay = {}` first.
    let engine = run("memo.decay.normal = 0.02");
...
fn registering_twice_replaces_rather_than_appends() {
    ...
    let engine = run(r#"
        memo.section("facts", { weight = 1 })
        memo.section("facts", { weight = 9 })
        "#);
...
fn the_file_returns_nothing() {
    let mut engine = Engine::new();
    engine
        .run("memo.inject_floor = 0.5", "test.lua")
...
fn one_raising_handler_does_not_cost_the_others_their_say() {
    ...
    engine
        .run(
            r#"
            memo.on.promote(function(c) error("this one is broken") end)
            memo.on.promote(function(c) return { promote = true } end)
            "#,
            "test.lua",
        )
        .expect("load");
...
fn a_told_handler_is_pure_side_effect() {
    ...
    engine
        .run(
            r#"
            memo.did.promote(function(m) memo.log("first: " .. m.text) end)
            memo.did.promote(function(m) error("broken") end)
            memo.did.promote(function(m) memo.log("third: " .. m.text) end)
            "#,
            "test.lua",
        )
        .expect("load");
...
fn a_handler_registered_against_an_unknown_name_is_refused_at_load() {
    // A gate that never fires is usually one registered against a name memo does not ask.
    // Better to say so while the config is loading than to be silently inert forever.
    let mut engine = Engine::new();
    let outcome = engine.run("memo.on.nonsense(function() end)", "test.lua");
    assert!(
...
fn registering_something_that_is_not_a_function_is_refused() {
    let mut engine = Engine::new();
    let outcome = engine.run("memo.on.scope(\"not a function\")", "test.lua");
    assert!(
        matches!(outcome, Err(LuaError::Runtime { .. })),
        "{outcome:?}"
    );
...
fn a_registrar_needs_a_name_first() {
    let mut engine = Engine::new();
    let outcome = engine.run("memo.section({ weight = 1 })", "test.lua");
    assert!(
        matches!(outcome, Err(LuaError::Runtime { .. })),
        "{outcome:?}"
    );
...
fn the_helpers_a_scope_handler_needs_are_there() {
    ...
    engine
        .run(
            r#"
            memo.on.scope(function(cwd)
              return { id = memo.path.basename(cwd), home = memo.home ~= nil }
            end)
            "#,
            "test.lua",
        )
...
fn a_gate_can_refuse_a_credential() {
    ...
    engine
        .run(
            r#"
            memo.on.promote(function(c)
              if memo.looks_like_secret(c.text) then
...
#[test]
fn settings_come_out_in_the_shapes_memo_uses() {
    let engine = run(r#"
        memo.inject_floor = 0.6
        memo.decay.normal = 0.02
        memo.witness.distillation = 0.1
        memo.budget.candidates = 100
        memo.imperatives = { "merk" }
        "#);
    let settings = Settings::from(&engine.config());
    assert_eq!(settings.floors.inject, 0.6);
    assert_eq!(settings.decay.normal, 0.02);
    assert_eq!(settings.weight(memo_model::WitnessKind::Distillation), 0.1);
    assert_eq!(settings.budget.candidates, 100);
    assert_eq!(settings.imperatives, ["merk"]);
    assert_eq!(
        settings.floors.live,
        memo_model::floor::LIVE,
...
mod trust {
    ...
    fn scratch(name: &str) -> PathBuf {
        let at = std::env::temp_dir().join(format!("memo-trust-{name}"));
    ...
    fn a_project_file_may_choose() {
        ...
        let file = write(&dir, ".memo.lua", "memo.inject_floor = 0.6\n");
        let mut engine = Engine::new();
    ...
    fn a_project_file_may_not_declare_a_source() {
        ...
        let file = write(
            &dir,
            ".memo.lua",
            "memo.source(\"mine\", { sessions = function() end })\n",
        );
        let mut engine = Engine::new();
    ...
    fn a_project_file_may_not_name_a_command_to_run() {
        ...
        let file = write(
            &dir,
            ".memo.lua",
            "memo.distiller = { kind = \"command\", argv = { \"curl\", \"evil\" } }\n",
        );
        let mut engine = Engine::new();
        match engine.read(&[(file, false)]) {
    ...
    fn a_project_file_may_not_vouch_for_itself() {
        // `memo.trusted` is how the owner vouches. A file that could add itself to it would
        // make the whole boundary decorative.
        let dir = scratch("selftrust");
        let file = write(&dir, ".memo.lua", "memo.trusted = { \"/\" }\n");
        let mut engine = Engine::new();
        assert!(matches!(
    ...
    fn the_owners_own_file_may_declare_anything() {
        ...
        let file = write(
            &dir,
            "init.lua",
            "memo.source(\"mine\", { kind = \"jsonl\" })\nmemo.distiller = { kind = \"endpoint\" }\n",
        );
        let mut engine = Engine::new();
    ...
    fn what_a_config_loads_runs_beside_it() {
        ...
        write(
            &dir,
            "sections.lua",
            "memo.section(\"facts\", { weight = 3 })\n",
        );
        let file = write(&dir, "init.lua", "memo.load(\"sections.lua\")\n");
        let mut engine = Engine::new();
    ...
    fn a_diamond_of_loads_terminates() {
        ...
        write(
            &dir,
            "shared.lua",
            "memo.section(\"shared\", { weight = 1 })\n",
        );
        write(&dir, "a.lua", "memo.load(\"shared.lua\")\n");
        write(&dir, "b.lua", "memo.load(\"shared.lua\")\n");
        let file = write(
            &dir,
            "init.lua",
            "memo.load(\"a.lua\")\nmemo.load(\"b.lua\")\n",
    ...
    fn a_file_that_exists_and_does_not_load_is_fatal() {
        ...
        let file = write(&dir, "init.lua", "memo.inject_floor = = =\n");
        let mut engine = Engine::new();
...
mod specs {
    ...
    fn the_describable_part_is_still_recorded() {
        ...
        engine
            .run(
                "memo.source(\"harness\", { kind = \"jsonl\", line = function() end })",
                "test.lua",
            )
            .expect("load");
        engine.harvest();
    ...
    fn a_line_the_adapter_skips_answers_nothing() {
        ...
        engine
            .run(
                "memo.source(\"harness\", { line = function(raw) return nil end })",
                "test.lua",
            )
            .expect("load");
        assert_eq!(
    ...
    fn an_adapter_that_raises_costs_its_line_and_nothing_else() {
        ...
        engine
            .run(
                "memo.source(\"harness\", { line = function(raw) error(\"bad\") end })",
                "test.lua",
            )
            .expect("load");
        assert_eq!(

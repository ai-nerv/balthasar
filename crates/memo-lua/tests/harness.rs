//! What a harness installs, and what happens to it when memo is not there.
//!
...
use memo_lua::{CLIENT, Engine};
use std::path::PathBuf;

/// The drop-in a harness loads, as it ships.
fn shipped() -> String {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../config/harness/memo.lua");
    std::fs::read_to_string(&path).expect("memo ships a harness drop-in")
}
...
fn the_drop_in_loads() {
    assert_eq!(through("harness._NAME"), "memo.harness");
}
...
#[test]
fn observing_with_no_memo_is_not_an_error() {
...
#[test]
fn planning_with_no_memo_answers_nothing() {
    // The whole degradation story: not an error to handle, an absence to carry on through.
    // A harness that gets nil does whatever it did before memo existed.
    assert_eq!(through("harness.plan('s', {}) == nil"), "true");
}

#[test]
fn every_call_survives_memo_being_absent() {
    for call in [
        "harness.plan('s', {})",
...
fn opening_without_the_client_says_what_is_missing() {
    // The usual cause is that `memo configs` has not been run, and saying so is what makes

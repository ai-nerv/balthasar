//! `memo forget --session` against the real binary, over the real file layout.
//!
...
use memo_model::{Body, Memory, NoteKind, ScopeId, SessionId, Tier};
use memo_store::mint;
use std::path::{Path, PathBuf};
use std::process::Command;

const NOW: memo_model::Timestamp = 1_756_000_000;
const SECRET: &str = "the deploy token MEMO_DEPLOY_TOKEN is hunter2-nevershare";
...
fn planted(at: &Path, session: &SessionId) -> (PathBuf, PathBuf, PathBuf) {
    ...
    let mut store = memo_store::Store::open(&memory).expect("store");
    store
    ...
    let mut pad = memo_store::Scratchpad::at(&runs);
...
fn forgetting_a_run_leaves_none_of_it_on_disk() {
    let at = std::env::temp_dir().join(format!("memo-cli-forget-{}", std::process::id()));
...
fn forgetting_a_run_nobody_has_heard_of_says_so() {
    ...
    let at = std::env::temp_dir().join(format!("memo-cli-typo-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&at);
    std::fs::create_dir_all(&at).expect("workspace");

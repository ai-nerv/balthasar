...
fn three_families_still_answer_with_no_embedder() {
    // §8.9: `AEON_NO_EMBED=1` retains temporal, causal-rule and entity traversal. Nothing in
...
fn rebuilding_a_derivation_leaves_every_memory_untouched() {
    ...
    store
        ...
        .expect("relate");
    store
        .retire_relations(Derivation::Rule, aeon_distil::RELATION_DERIVATION, NOW + 1)
        .expect("retire");
...
fn a_dense_store_stays_within_its_bounds() {
    ...
    let edges: Vec<aeon_model::Relation> = (0..2000)
        .map(|n| aeon_model::Relation {
            from: hub.clone(),

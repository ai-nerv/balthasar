...
use aeon_distil::{Ingest, ingest};
use aeon_lua::{Engine, Settings};
use aeon_model::{ScopeId, Tier, Timestamp, WitnessKind};
use aeon_store::{Recall, Store};
...
/// Every source adapter aeon ships, loaded, with the ids they registered.
fn shipped() -> (Engine, Vec<String>) {
    let mut engine = Engine::new();
    let files: Vec<(PathBuf, bool)> =
        aeon_lua::glob_paths(&format!("{}/sources/*.lua", config_dir().display()))
            .into_iter()
            .map(|p| (PathBuf::from(p), true))
            .collect();
    assert!(!files.is_empty(), "aeon ships at least one source adapter");
    engine
        .read(&files)
...
fn journal(name: &str, lines: &[String]) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("aeon-ingest-{name}"));
...
fn ingest_one(source: &str, file: &Path, store: &mut Store, dry_run: bool) -> aeon_distil::Report {
    let (mut engine, _) = shipped();
    // Keep the adapter's own `meta` and `line`, and give it one file to walk.
    engine
        .run(
            &format!(
                "local held = __aeon_specs.source[{source:?}]\n\

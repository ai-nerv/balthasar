...
use aeon_model::{Memory, ScopeId, SessionId, Timestamp, Witness, WitnessId};
use aeon_store::{Landing, Store, mint};
...
fn write(
    ...
    source: &Source,
    candidate: &Candidate,
    importance: aeon_model::Importance,
    pinned: bool,
    ...
    memory.temporal = aeon_model::Temporal::recalled(ask.now, happened);
    memory.session = Some(session.clone());
    memory.strength.importance = importance;
    memory.strength.pinned = pinned;
    memory.provenance = aeon_model::Provenance {
        through: aeon_model::Through::Ingest,

...
//! by where aeon is standing: running under a harness, the cheapest correct answer is to ask
...
//! backend produced it, so `aeon why` can say "this came out of the rules, not a model".
...
/// client, no credential handling in aeon at all, and it inherits whatever the person has
...
/// Read a configuration's `aeon.distiller` into backends, in the order it listed them.
///
...
pub fn backends(said: Option<&serde_json::Value>) -> (Vec<Box<dyn Distil>>, Vec<String>) {
    ...
    for spec in specs {
        match spec.get("kind").and_then(serde_json::Value::as_str) {
            ...
            Some(other) => unavailable.push(format!("'{other}' is not a backend aeon knows")),

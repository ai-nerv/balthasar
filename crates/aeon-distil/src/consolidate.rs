...
use aeon_store::{Cluster, Landing, Store, mint};
...
fn promote(
    ...
    memory.temporal = aeon_model::Temporal::recalled(now, group.first_seen);
    memory.session = group.sessions.first().cloned();
    memory.provenance = aeon_model::Provenance {
        through: aeon_model::Through::Sleep,
        who: None,
    };
    ...
    // makes `aeon why` able to print the argument rather than a number.

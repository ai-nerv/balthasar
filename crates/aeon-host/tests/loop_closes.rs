...
//! what it ran and how it went. aeon works out for itself whether the action followed anything
...
use aeon_ipc::{Peer, Reply, Request};
use aeon_model::{
...
fn an_action_that_followed_a_memory_attributes_itself() {
    // The centre of it. The harness says what it ran; aeon works out that the memory naming
    ...
    assert_eq!(
        ...
        aeon_model::Attribution::Structural,
        "aeon did not see the match for itself"
    );
...
fn an_unrelated_action_credits_nothing() {
    ...
    assert_eq!(recorded.attribution, aeon_model::Attribution::Proximal);
}

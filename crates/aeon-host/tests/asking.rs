...
use aeon_host::{Door, answer};
use aeon_ipc::{Peer, Request};
use aeon_model::{ScopeId, Timestamp, WitnessKind, floor};
...
fn value(reply: &aeon_ipc::Reply) -> serde_json::Value {
    reply
        .result
        .as_ref()
        .and_then(|r| r.first())
...
fn a_peer_may_not_forge_an_imperative_by_asking_for_one() {
    ...
    let memory = held
        .0
        .get(&aeon_model::MemoryId::new(id))
        .expect("get")
        .expect("there");
...
fn every_write_by_a_peer_says_which_process() {
    // `aeon why` has to be able to answer "which process believes this".
    let mut held = Held::new();
    let reply = held.ask(
        &peer(),

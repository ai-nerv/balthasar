...
//!
//! Verified by having a *different* VM connect, never by aeon talking to itself. Every
...
fn one_connection_carries_many_calls() {
    // aeon is asked several times per turn, which is why the handle is held. A server that

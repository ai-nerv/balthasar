...
use aeon_model::{ScopeId, SessionId, floor};
use aeon_store::{Scratchpad, Store, Transcript};
use std::path::PathBuf;

const NOW: aeon_model::Timestamp = 1_756_000_000;

fn scratch(name: &str) -> PathBuf {
    let at = std::env::temp_dir().join(format!("aeon-layout-{name}"));
...
fn deleting_one_run_is_deleting_one_directory() {
    ...
    std::fs::remove_dir_all(aeon_store::session_dir_in(&home, &one)).expect("rm");

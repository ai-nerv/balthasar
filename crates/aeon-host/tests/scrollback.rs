...
use aeon_host::{Answering, Door, answer};
use aeon_ipc::{Reply, Request};
use aeon_model::{ScopeId, floor};
use aeon_store::{Store, Transcript};
...
fn what_a_harness_wrote_comes_back_byte_for_byte() {
    // The whole claim. aeon stores the record and never parses it, so a harness gets back
...
fn why_quotes_the_turn_a_witness_saw() {
    ...
    let mut memory = aeon_model::Memory::new(
        aeon_store::mint(NOW),
        aeon_model::Tier::Fact,
        ScopeId::new("/w/thing"),
        aeon_model::Body::note("we deploy to fly.io", aeon_model::NoteKind::Claim),
        NOW,
    );
    memory.session = Some(aeon_model::SessionId::new(S));
    let witness = aeon_model::Witness::new(
        aeon_model::WitnessId::new("w1"),
        aeon_model::WitnessKind::Imperative,
        aeon_model::SessionId::new(S),
        ScopeId::new("/w/thing"),
        NOW,
    )
    .at_cursor(7);
...
fn a_host_with_no_scrollback_says_so_rather_than_pretending() {
    // A harness relying on aeon for persistence must find out immediately, not discover on

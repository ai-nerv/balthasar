...
//!
//! Skipped unless `MEMO_MODEL_DIR` names a directory holding a sentence transformer, because a
...
use memo_embed::{Dense, Embed, Hashed};

fn model() -> Option<Dense> {
    let dir = std::env::var("MEMO_MODEL_DIR").ok()?;

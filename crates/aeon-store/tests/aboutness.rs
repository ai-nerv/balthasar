...
use aeon_store::{Landing, Recall, Store, mint};
...
fn a_query_about_a_thing_finds_it_without_sharing_a_word() {
    ...
    keep(
        &mut store,
        Body::note("we ship to fly.io now", aeon_model::NoteKind::Claim),
        "w1",
    );
...
fn a_rare_thing_counts_for_more_than_a_common_one() {
    ...
    keep(
        &mut rare,
        Body::note("the staging box runs Redis", aeon_model::NoteKind::Claim),
        "w1",
    );
...
fn one_projects_entities_do_not_reach_another() {
    ...
    keep(
        &mut store,
        Body::note("we use Redis", aeon_model::NoteKind::Claim),
        "w1",
    );
...
fn a_predicate_naming_a_set_accumulates() {
    ...
    // so aeon could not record that somebody liked two things.
    let mut store = store();

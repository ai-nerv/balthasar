...
use aeon_store::{Reach, Recall, Store, mint};

const NOW: aeon_model::Timestamp = 1_756_000_000;
const SECRET: &str = "the deploy token AEON_DEPLOY_TOKEN is hunter2-abcdef-nevershare";
...
fn a_store_with_a_secret() -> (Store, aeon_model::MemoryId, aeon_model::MemoryId) {
...
fn a_purge_says_what_it_will_take_with_it() {
    ...
    let closure = aeon_store::closure_of(&store, &secret).expect("closure");
...
fn purging_an_indexed_memory_does_not_fail() {
    ...
    let gone = aeon_store::purge(&mut store, &secret).expect("purge must not fail");
    assert_eq!(gone, 1);
}
...
fn nothing_ordinary_finds_a_purged_secret() {
    let (mut store, secret, _) = a_store_with_a_secret();
    aeon_store::purge(&mut store, &secret).expect("purge");

    for query in ["deploy token", "hunter2", "nevershare", "AEON_DEPLOY_TOKEN"] {

...
impl Scratchpad {
    /// Scratch beneath a tool's home — `<project>/aeon/<tool>`.
    #[must_use]
    ...
    /// and says nothing should leave nothing behind, and an `aeon/` tree full of empty
    ...
    pub fn recurring(
        ...
        scope: &str,
        at_least: usize,
        since: aeon_model::Timestamp,
        cap: usize,
    ) -> Result<Vec<crate::Cluster>, StoreError> {
        let mut seen: HashMap<String, (String, aeon_model::Timestamp, Vec<SessionId>)> =
            HashMap::new();
            ...
            let rows = statement
                .query_map(rusqlite::params![scope, since], |r| {
                    Ok((
                        r.get::<_, String>(0)?,
                        r.get::<_, String>(1)?,
                        r.get::<_, aeon_model::Timestamp>(2)?,
                        r.get::<_, String>(3)?,
                    ))
                })?
    ...
    /// corroboration in.
    pub fn weaken_all(&mut self, now: aeon_model::Timestamp) -> Result<usize, StoreError> {
        let mut faded = 0;
    ...
    pub fn sweep_all(&mut self, now: aeon_model::Timestamp) -> Result<usize, StoreError> {
        let mut swept = 0;
        for path in self.runs() {
            let mut store = Store::open(&path)?;
    ...
    pub fn carry(
        project: &mut Store,
        run: &mut Store,
        held: aeon_model::Memory,
        witness: aeon_model::Witness,
        at: aeon_model::Timestamp,
    ) -> Result<crate::Landing, StoreError> {
        let was = held.id.clone();
        let mut moving = held;
        moving.tier = aeon_model::Tier::Fact;
...
mod tests {
    use super::*;
    use aeon_model::{Body, Memory, NoteKind, ScopeId, Tier, Witness, WitnessKind};

    const NOW: aeon_model::Timestamp = 1_700_000_000;

    fn scratch(name: &str) -> PathBuf {
        let at = std::env::temp_dir().join(format!("aeon-pad-{name}"));
    ...
    fn said_by(session: &SessionId) -> Witness {
        Witness::new(
            aeon_model::WitnessId::new(crate::mint(NOW).as_str()),
    ...
    fn a_carried_memory_stops_being_the_runs_to_offer() {
        // Otherwise `aeon promote` keeps offering the same memory after it has crossed.

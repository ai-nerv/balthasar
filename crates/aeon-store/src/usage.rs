...
use aeon_model::{Attribution, MemoryId, OutcomeKind, Presentation, ScopeId, SessionId, Timestamp};
use rusqlite::params;
...
impl Store {
    ...
    pub fn note_use(&mut self, used: &Use) -> Result<(), StoreError> {
        // Checked before the write, so an action against a context this aeon never assembled
        ...
        if let Some(injection) = &used.injection {
            ...
            if known == 0 {
                return Err(StoreError::Unknown(format!(
                    "injection called '{injection}' — this aeon never assembled one"
                )));
            }
        }
        let tx = self.db_mut().transaction()?;
    ...
    pub fn utility_of(&self, memory: &MemoryId) -> Result<aeon_model::Utility, StoreError> {
        let mut held = aeon_model::Utility::default();

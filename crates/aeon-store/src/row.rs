...
/// Anything with a `FromStr` whose failure means the file was written by a different aeon.
...
mod tests {
    ...
    #[test]
    fn a_row_written_by_a_different_aeon_says_so() {
    ...
    fn only_a_fact_fills_the_indexed_columns() {
        ...
        let (s, _, _) = slot(&Body::note("hi", aeon_model::NoteKind::Scratch));
        assert_eq!(s, None);
    }
}

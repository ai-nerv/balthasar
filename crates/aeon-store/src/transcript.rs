...
//!
//! **What a turn holds is opaque.** aeon keeps the harness's own record as a string it never
//! parses, alongside a small projection it does understand — enough to quote a turn and to
//! search one. A harness gets back exactly what it wrote; aeon never needs to know what an
//! `Entry` is, and commitment 1 survives contact with being the only place the data lives.

use crate::StoreError;
use aeon_model::{SessionId, Timestamp};
...
pub struct Turn {
    /// Where in the session. The harness's own numbering, never aeon's.
    pub cursor: u64,
    /// When it settled.
    pub at: Timestamp,
    /// Who said it.
    ...
    ///
    /// Opaque. aeon stores it, hands it back, and never looks inside — which is what lets a
    /// harness treat this as its journal without aeon knowing what its records mean.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[must_use]
pub fn transcript_path(scope: &aeon_model::ScopeId, tool: &crate::Tool) -> PathBuf {
...
impl Transcript {
    ...
    ///
    /// The reason `aeon why` can show what a witness saw rather than naming a number at it.
...
mod tests {
    ...
    fn what_a_harness_wrote_comes_back_untouched() {
        // aeon stores the record and never parses it. That is what lets a harness treat this
        // as its journal without aeon knowing what its records mean.
        let mut t = held();
        let s = SessionId::new("s1");
    ...
    fn the_scrollback_lives_beside_the_memory_but_not_in_it() {
        ...
        let scope = aeon_model::ScopeId::new("/w/thing");

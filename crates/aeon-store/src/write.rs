...
impl Store {
    ...
    fn standing(&self, memory: &Memory) -> Result<Option<MemoryId>, StoreError> {
        ...
        let found: Option<String> = match (subject, predicate) {
            ...
            (Some(subject), Some(predicate)) if aeon_model::is_single_valued(&predicate) => self
                .db()
                .query_row(
    ...
    fn revised_by(&self, memory: &Memory) -> Result<Option<String>, StoreError> {
        if memory.tier != aeon_model::Tier::Fact {
            return Ok(None);
        }
        let text = memory.text();
        let lead = aeon_model::lead(&text);
        if lead.is_empty() {
            return Ok(None);
        }
        ...
        Ok(held
            .into_iter()
            .find(|(_, held)| aeon_model::same_claim_different_value(held, &text))
            .map(|(id, _)| id))
    }

    /// Whether what is already there says the same thing as what arrived.
    fn says_the_same(&self, existing: &MemoryId, arriving: &Memory) -> Result<bool, StoreError> {
        let hash: String = self.db().query_row(
            "SELECT content_hash FROM memory WHERE id = ?1",
            params![existing.as_str()],
            |r| r.get(0),
        )?;
        Ok(hash == arriving.content_hash)
    }
    ...
    fn write_row(
        ...
        let single = predicate
            .as_deref()
            .is_none_or(aeon_model::is_single_valued);
        // A claim with no slot still has an opening. It is the bucket a later revision of the
        // same claim is looked up in.
        let lead = (memory.tier == aeon_model::Tier::Fact && predicate.is_none())
            .then(|| aeon_model::lead(&memory.text()))
            .filter(|l| !l.is_empty());
    ...
    /// argument disagree is the one state `aeon why` must never be able to show.
...
impl Store {
    ...
    pub fn promote(
        ...
        // learned it is part of what `aeon why` has to be able to say.
        self.db().execute(
    ...
    ///
    /// What `aeon promote` is choosing from, and what dies with the session if nobody does.
    pub fn uncrossed(&self, session: &aeon_model::SessionId) -> Result<Vec<Memory>, StoreError> {
...
mod promoting {
    ...
    use crate::Store;
    use crate::mint::mint;
    use aeon_model::{Body, NoteKind, ScopeId, SessionId, Tier, WitnessId, WitnessKind};
    ...
    fn what_a_person_keeps_becomes_the_projects() {
        ...
        assert!(confidence > aeon_model::floor::INJECT, "{confidence}");
        assert!(held.is_assertable(aeon_model::floor::INJECT, NOW, false));
    }

    #[test]
    fn it_still_says_which_run_learned_it() {
        // A durable memory belongs to the project; which session learned it is part of what
        // `aeon why` has to be able to answer.
        let mut store = Store::ephemeral().expect("store");
        let id = scratch(&mut store, "the deploy target is fly.io");
        store
            .promote(&id, Tier::Fact, asked_for(), NOW)

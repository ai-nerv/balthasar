...
//! to a real socket.
//!
//! Not memo calling itself. The stub goes into a VM as *source*, is handed nothing but the
...
use memo_ipc::{Listener, Peer, Request};
...
fn serving(name: &str, seed: &[&str]) -> (std::path::PathBuf, std::thread::JoinHandle<()>) {
    ...
    let thread = std::thread::spawn(move || {
        let _ = listener.serve(|peer: &Peer, request: Request| {
            ...
            memo_host::answer(&mut at, &Door::Socket(peer.clone()), &request)
        });
    });
    (path, thread)
}

/// Run a script with the stub loaded and connected, and take what it left in `memo.answer`.
fn through_the_stub(path: &std::path::Path, script: &str) -> String {
    ...
    let source = format!(
        ...
        local client = chunk(memo.stream)
        local mem, why = client.connect({{ path = {:?} }})
        if not mem then memo.answer = "could not connect: " .. tostring(why) return end
...
fn the_stub_says_so_when_nothing_is_listening() {
    ...
    let source = format!(
        ...
        memo.answer = tostring(mem == nil and type(why) == "string")
        "#
    );

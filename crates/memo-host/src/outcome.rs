...
use memo_model::{Attribution, MemoryId, OutcomeKind, SessionId};
use memo_store::{Use, Verdict};
...
pub fn used(at: &mut Answering<'_>, door: &Door, request: &Request) -> Reply {
    ...
    // followed this memory" is explicit; a peer claiming a structural match memo did not
    ...
    // What memo can see for itself. A caller reporting "I ran `make test`" after being handed
    ...
    let held = Use {
        ...
        action_hash: memo_model::content_hash(&ran)[..16].to_owned(),
        attribution,
        memories,
    };
...
/// runs `make test` are related in a way memo can check, so it checks rather than asking — which

...
//! of that evidence, and the reason `memo why` can print an argument rather than a number.
...
pub struct Witness {
    ...
    /// When.
    pub at: Timestamp,
    /// Where in that session's transcript, so `memo why` can point at it.
...
impl Witness {
    ...
    /// Say how this reached memo, and where it came from.
    ///
...
impl fmt::Display for UnknownKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "'{}' is not a witness memo knows", self.0)
    }
}
...
mod tests {
    ...
    fn distillation_alone_does_not_reach_the_promotion_floor() {
        // The main defence against "the model said it once and memo believes it forever":

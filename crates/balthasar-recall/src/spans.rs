...
//!
//! A span has no witnesses and no derived confidence, so nothing here asserts one.

use balthasar_store::{Span, Transcript};
...
///
/// `already` is the text of everything memory is already sending; a restatement is dropped.
pub fn quote(
    held: &Transcript,
    query: &str,
    already: &[String],
...
) -> Result<Quotes, balthasar_store::StoreError> {
    let mut out = Quotes::default();
    if budget_tokens == 0 || query.trim().is_empty() {
        return Ok(out);
    }

    for span in held.spans_matching(&balthasar_store::fts_query(query), CONSIDERED)? {
        let text = span.text.trim();
        if text.len() < 12 {
            continue;
        }
        // Reads like an instruction rather than a description. Findable, never injected.
        ...
        if already
            .iter()
            .any(|held| balthasar_model::same_claim(held, text))
        {
            out.deduplicated += 1;
            continue;
        }
...
mod tests {
    ...
    fn the_boundary_handler_sees_every_span() {
        // A span is raw text and can carry a secret nobody ever promoted.

...
) -> Result<Context, balthasar_store::StoreError> {
...
fn gather(
    ...
    section: &Section,
    ask: &Ask,
) -> Result<Vec<Memory>, balthasar_store::StoreError> {
...
pub fn stores_for(
    scope: &ScopeId,
    open: impl Fn(&ScopeId) -> Result<Store, balthasar_store::StoreError>,
) -> Result<Vec<(Store, bool)>, balthasar_store::StoreError> {
    if scope.is_global() {

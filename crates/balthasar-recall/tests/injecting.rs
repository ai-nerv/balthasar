...
use balthasar_recall::{Ask, Bound, Section, assemble};
use balthasar_store::{Store, Weights, mint};
...
fn the_same_claim_in_two_stores_is_still_said_once() {
    ...
    keep(
        ...
        Body::note(
            "we run the tests with make test",
            balthasar_model::NoteKind::Claim,
        ),
        Importance::High,
        WitnessKind::Imperative,
    );
...
fn a_line_a_handler_withholds_is_not_said() {
    ...
    keep(
        &mut store,
        Tier::Fact,
        Body::note("the token is sk-secret", balthasar_model::NoteKind::Claim),
        Importance::High,
        WitnessKind::Imperative,
    );
    keep(
        &mut store,
        Tier::Fact,
        Body::note("we deploy with fly", balthasar_model::NoteKind::Claim),
        Importance::High,
        WitnessKind::Imperative,
    );
    ...
    for (n, when) in [(1, NOW - 300), (2, NOW - 200), (3, NOW - 100)] {
        let mut memory = Memory::new(
            ...
            Body::episode(
                ...
                balthasar_model::Span::at(n),
                vec![],
                balthasar_model::Outcome::Done,
            ),
            when,
        );
        memory.temporal = balthasar_model::Temporal::recalled(NOW, when);
        let witness = Witness::new(

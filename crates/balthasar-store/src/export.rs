...
//!
//! Explicit and never automatic. balthasar does not start training jobs, does not phone anywhere,
...
/// Deserialisable as well as serialisable: `balthasar dataset` writes these and `balthasar train` reads
...
impl Store {
    ...
    pub fn training_rows(&self, limit: usize) -> Result<Vec<Row>, StoreError> {
        ...
        ) in rows
        {
            ...
            let id = balthasar_model::MemoryId::new(memory);
...
mod tests {
    ...
    fn a_full_trail() -> (Store, balthasar_model::MemoryId) {
    ...
    fn a_candidate_nobody_acted_on_is_still_a_row() {
        ...
        store
            .note_recall(
                ...
                &[crate::Candidate {
                    memory: balthasar_model::MemoryId::new("never-used"),
                    rank: 3,
                    selected: false,
                    score: 0.1,

...
impl Store {
    ...
    fn write_row(
        ...
        let single = predicate
            .as_deref()
            .is_none_or(balthasar_model::is_single_valued);
        // A claim with no slot still has an opening. It is the bucket a later revision of the
        // same claim is looked up in.
        let lead = (memory.tier == balthasar_model::Tier::Fact && predicate.is_none())
            .then(|| balthasar_model::lead(&memory.text()))
            .filter(|l| !l.is_empty());
    ...
    /// argument disagree is the one state `balthasar why` must never be able to show.
    ...
    pub fn link(
        ...
        src: &MemoryId,
        dst: &MemoryId,
        rel: balthasar_model::LinkRelation,
        at: Timestamp,
    ) -> Result<(), StoreError> {
        self.db().execute(
    ...
    pub fn db_privacy(
        &mut self,
        id: &MemoryId,
        privacy: balthasar_model::Privacy,
    ) -> Result<(), StoreError> {
        self.db().execute(
...
fn write_witness(
    ...
    tx.execute(
        ...
        params![
            ...
            witness.domain.as_ref().map(balthasar_model::Domain::as_str),
        ],
    )?;
    Ok(())
}
...
impl Store {
    ...
    pub fn promote(
        ...
        // learned it is part of what `balthasar why` has to be able to say.
        self.db().execute(
    ...
    ///
    /// What `balthasar promote` is choosing from, and what dies with the session if nobody does.
    pub fn uncrossed(
        &self,
        session: &balthasar_model::SessionId,
...
mod promoting {
    ...
    use crate::Store;
    use crate::mint::mint;
    use balthasar_model::{Body, NoteKind, ScopeId, SessionId, Tier, WitnessId, WitnessKind};
    ...
    fn what_a_person_keeps_becomes_the_projects() {
        ...
        assert!(confidence > balthasar_model::floor::INJECT, "{confidence}");
        assert!(held.is_assertable(balthasar_model::floor::INJECT, NOW, false));
    }

    #[test]
    fn it_still_says_which_run_learned_it() {
        // A durable memory belongs to the project; which session learned it is part of what
        // `balthasar why` has to be able to answer.
        let mut store = Store::ephemeral().expect("store");
        let id = scratch(&mut store, "the deploy target is fly.io");
        store
            .promote(&id, Tier::Fact, asked_for(), NOW)

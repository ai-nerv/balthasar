...
use balthasar_store::{Injection, Store, Use, Verdict, mint};

const NOW: balthasar_model::Timestamp = 1_756_000_000;
const DAY: balthasar_model::Timestamp = 86_400;

fn scope() -> ScopeId {
    ScopeId::new("/w/thing")
}

fn kept(store: &mut Store, tier: Tier, text: &str) -> balthasar_model::MemoryId {
    let held = Memory::new(
        mint(NOW),
        tier,
        scope(),
...
fn staleness_is_about_validity_not_about_being_ignored() {
    ...
    assert!(!balthasar_model::is_stale(NOW - 200 * DAY, None, NOW));
    assert!(balthasar_model::is_stale(
        NOW - 10 * DAY,
        Some(NOW - DAY),
        NOW
    ));

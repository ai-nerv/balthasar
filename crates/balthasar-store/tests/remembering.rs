...
use balthasar_store::{Landing, Recall, Store, mint};
...
fn both_witnesses_survive_a_correction() {
    // `balthasar why` has to be able to show the argument on either side of a change of mind.
    let mut store = store();
    store
        .remember(
            fact("project", "deploy", "heroku", MARCH),
...
fn one_claim_restated_lands_on_the_claim_already_held() {
    ...
    assert!(
        matches!(restated, balthasar_store::Landing::Reinforced(_)),
...
fn a_query_of_only_short_words_answers_nothing_rather_than_failing() {
    ...
    // `balthasar recall "is it a"` did not answer nothing; it errored with "unterminated string".
    let mut store = store();
    store
        .remember(

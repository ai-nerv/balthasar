...
//!
//! balthasar is the only copy of what was said, so a run's transcript grows without limit while a
...
use balthasar_store::{Budget, Transcript, Turn, Want};
...
fn a_citation_comes_back_with_context_on_both_sides() {
    // What `balthasar why` needs: the quoted turn is useless without what surrounded it.
    let held = a_long_run(100);
    let read = held
        .read(
            &run(),

...
use memo_recall::{Ask, Bound, Section, assemble};
use memo_store::{Store, Weights, mint};
...
fn a_line_a_handler_withholds_is_not_said() {
    ...
    keep(
        &mut store,
        Tier::Fact,
        Body::note("the token is sk-secret", memo_model::NoteKind::Claim),
        Importance::High,
        WitnessKind::Imperative,
    );
    keep(
        &mut store,
        Tier::Fact,
        Body::note("we deploy with fly", memo_model::NoteKind::Claim),
        Importance::High,
        WitnessKind::Imperative,
    );
    ...
    for (n, when) in [(1, NOW - 300), (2, NOW - 200), (3, NOW - 100)] {
        let mut memory = Memory::new(
            ...
            Body::episode(
                ...
                memo_model::Span::at(n),
                vec![],
                memo_model::Outcome::Done,
            ),
            when,
        );
        memory.temporal = memo_model::Temporal::recalled(NOW, when);
        let witness = Witness::new(

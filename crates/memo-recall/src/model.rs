...
mod tests {
    ...
    fn every_way_of_having_no_model_is_the_same_way() {
        ...
        let missing = std::env::temp_dir().join("memo-no-such-model-9f3a.json");
        let _ = std::fs::remove_file(&missing);
        assert!(Model::load(&missing).is_none());

        let junk = std::env::temp_dir().join("memo-junk-model.json");
        std::fs::write(&junk, "not json at all").expect("write");
        assert!(Model::load(&junk).is_none());

        let weak = std::env::temp_dir().join("memo-weak-model.json");
        let mut held = a_model();
    ...
    fn a_good_model_survives_a_round_trip() {
        let at = std::env::temp_dir().join("memo-good-model.json");
        let held = a_model();

...
impl Strength {
    ...
    ///
    /// Separate from [`Self::decay`] so `aeon decay --preview` can show what a pass would do
...
impl fmt::Display for UnknownImportance {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "'{}' is not an importance aeon knows", self.0)
    }
}
...
mod tests {
    ...
    fn previewing_does_not_change_anything() {
        // `aeon decay --preview` has to be able to show the future without causing it.

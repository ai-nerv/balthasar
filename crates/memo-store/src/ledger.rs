...
//! `memo-buffer`'s, which is the arrangement that lets a plan be rehearsed without a store to
//! rehearse it against.

use crate::{Store, StoreError};
use memo_model::{MemoryId, SessionId, Timestamp};
use rusqlite::params;
use std::fmt;
use std::str::FromStr;
...
pub struct Entry {
    /// The harness's own index. memo never renumbers, so a plan can be applied without a
    ...
    pub fn observe(&mut self, session: &SessionId, entry: &Entry) -> Result<(), StoreError> {
        self.db().execute(
            ...
            params![
                session.as_str(),
                entry.cursor as i64,
                entry.memory.as_ref().map(memo_model::MemoryId::as_str),
                entry.role,
                entry.kind,
                entry.tool,
                entry.tokens,

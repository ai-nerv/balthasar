...
//!
//! ```text
//! <project>/memo/<tool>/project.db          the checkout's memory, for that tool
//! <project>/memo/<tool>/<session>/memory.db that run's scratch
//! <project>/memo/<tool>/<session>/transcript.db
//! ~/.local/share/memo/<tool>/global.db      yours, everywhere
//! ```
//!
...
pub const HOME: &str = "memo";

/// What marks one as memo's rather than a directory that happens to be called `memo`.
///
/// Without it, any directory containing a subdirectory named `memo` would be mistaken for a
/// project root — including the parent of memo's own checkout, which is how this was found.
const MARKER: &str = ".store";

/// What goes in the marker: enough to tell a later layout that this is an earlier one.
const MARKER_BODY: &str = "memo store layout 1\n";
...
/// The directory holding memo's own data.
///
/// `$XDG_DATA_HOME/memo`, falling back to `~/.local/share/memo`. Beside the data rather than in
...
pub fn data_dir() -> PathBuf {
    if let Some(xdg) = std::env::var_os("XDG_DATA_HOME").filter(|v| !v.is_empty()) {
        return PathBuf::from(xdg).join("memo");
    }
    if let Some(home) = std::env::var_os("HOME").filter(|v| !v.is_empty()) {
        return PathBuf::from(home).join(".local/share/memo");
    }
...
impl Tool {
    ...
    pub const DEFAULT: &'static str = "memo";
...
///
/// A project's own `memo/` when the scope is a checkout, so that memory moves with the code.
...
/// somebody ran `memo init`. Callers use it to decide whether there is a home to create: the
...
fn session_stem(session: &str) -> String {
    ...
    let digest = memo_model::content_hash(session);
...
fn file_stem(scope: &str) -> String {
    let leaf: String = scope
        ...
        .collect();
    let digest = memo_model::content_hash(scope);
...
/// itself. The first rule is what lets `memo init` in a subdirectory scope memory to a subtree
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let at = std::env::temp_dir().join(format!("memo-paths-{name}"));
    ...
    #[test]
    fn a_directory_named_memo_is_not_mistaken_for_a_store() {
        // Found the hard way: memo's own checkout is called `memo`, so without the marker the
    ...
    fn a_repository_keeps_its_memory_inside_itself() {
        ...
        assert_eq!(path, root.join("memo/memo/project.db"));
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn a_directory_in_no_repository_is_its_own_scope() {
        ...
        let nowhere = Path::new("/memo-no-such-root-9f3a/deep/inside");
    ...
    fn a_scope_with_no_project_keeps_its_memory_in_the_data_directory() {
        ...
        let nowhere = ScopeId::new("/memo-no-such-root-9f3a/deep");
    ...
    fn a_run_keeps_its_scratch_and_its_scrollback_in_one_directory() {
        ...
        let scope = ScopeId::new("/memo-no-such-root-9f3a/p");

...
use memo_model::{Link, Memory, MemoryId, Privacy, Tier, Timestamp, Witness};
...
pub struct Recall {
    ...
    /// nothing ever faded and `memo decay` always reported an empty pass. Inspection is not
...
impl Store {
    ...
    ///
    /// Newest first because that is the order `memo why` reads best in: what most recently
    ...
    /// Every memory, oldest first. What `memo export` walks.
    ...
    fn sessions_saying(
        ...
        scope: &str,
        hash: &str,
    ) -> Result<Vec<memo_model::SessionId>, StoreError> {
        let mut statement = self.db().prepare(
        ...
        let found = statement
            .query_map(params![scope, hash], |r| {
                Ok(memo_model::SessionId::new(r.get::<_, String>(0)?))
            })?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(found)
    }
...
mod topping_up {
    use super::*;
    use memo_model::{Body, Memory, ScopeId, SessionId, Tier, Witness, WitnessId, WitnessKind};

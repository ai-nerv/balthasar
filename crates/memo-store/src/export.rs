...
//!
//! Explicit and never automatic. memo does not start training jobs, does not phone anywhere,
...
/// Deserialisable as well as serialisable: `memo dataset` writes these and `memo train` reads
...
impl Store {
    ...
    pub fn training_rows(&self, limit: usize) -> Result<Vec<Row>, StoreError> {
        ...
        ) in rows
        {
            ...
            let id = memo_model::MemoryId::new(memory);
...
mod tests {
    ...
    fn a_full_trail() -> (Store, memo_model::MemoryId) {
    ...
    fn a_candidate_nobody_acted_on_is_still_a_row() {
        ...
        store
            .note_recall(
                ...
                &[crate::Candidate {
                    memory: memo_model::MemoryId::new("never-used"),
                    rank: 3,
                    selected: false,
                    score: 0.1,

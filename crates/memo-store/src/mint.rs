...
use memo_model::{MemoryId, Timestamp};
use std::sync::atomic::{AtomicU64, Ordering};

/// Distinguishes two ids minted in the same millisecond.
///
/// A counter rather than a random number: memo mints ids in one process, monotonic is a
...
mod mint_tests {
    ...
    /// The trailing eight characters, which is what `memo why` and `memo forget` are given.

...
impl Memory {
    ...
    /// whose number and whose argument disagree, which is the one state `balthasar why` must never
    /// be able to show.
    ...
    ///
    /// The diversity number, exposed because `balthasar why` prints it and a person reading one
...
mod tests {
    ...
    fn a_memory_round_trips_as_json() {
        // `balthasar export` is this, a line at a time. A record that will not serialise is a

...
///
/// The write ceiling is enforced on this: a socket peer cannot pin, cannot reach global, and
/// cannot claim an imperative.
...
pub struct Provenance {
    pub through: Through,
    /// The peer, when there was one, taken from the kernel and never from a name it sent.
...
pub struct Memory {
    ...
    ///
    /// Absent is the ordinary state, not a degraded one: recall works lexically.
...
impl Memory {
    ...
    ///
    /// Confidence starts at zero and stays there until evidence arrives.
    #[must_use]
    ...
    ///
    /// Below `floor` a memory stops being *asserted* long before it stops being *findable*.
    #[must_use]
    pub fn is_assertable(&self, floor: f64, now: Timestamp, remote: bool) -> bool {
        self.archived_at.is_none()
            ...
            // Faded past being worth keeping is faded past being worth stating.
            && self.strength.at_tier(self.tier, now) >= crate::floor::SPENT
            // Stale is not the same as faded: a fact barely decays, and a pinned memory is exempt.
            && (self.strength.pinned
    ...
    ///
    /// Not a delete, and there is no delete next to it.

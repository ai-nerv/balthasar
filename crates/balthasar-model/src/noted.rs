//! What a spawned helper said before it failed, and every step balthasar takes when somebody is
//! watching. The diagnosis goes to a file, and only when somebody asked for one: `$BALTHASAR_DEBUG_LOG`
//! for this program alone, `$NERV_LOG` for the whole family in one file.
...
/// Write one line to whichever logs were asked for, formatted like `println!`.
///
/// A macro rather than a function so the arguments are not evaluated when nobody is listening.
#[macro_export]
macro_rules! noted {
    ($($arg:tt)*) => {
...
mod tests {
    ...
    fn lines_are_appended_stamped_with_when_and_who() {
        ...
        let who = format!(" balthasar[{}] ", std::process::id());

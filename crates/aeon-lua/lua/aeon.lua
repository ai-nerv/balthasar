-- aeon's client library: what another program requires to talk to a memory layer.
--
--   local aeon = load(src)(my_transport)
--   local mem  = aeon.connect()
...
-- connection, or `io.popen("aeon lua-api"):read("a")` where the host permits `io`.
--
-- Plain Lua on purpose. It runs unchanged in aeon's own VM, in axon's, in hexe's, in oslo's,
...
--
-- Inside aeon that is `aeon.stream`, and it is found automatically when nothing is passed.
--
-- **The surface is small on purpose.** It is not a mirror of what aeon can do; it is the
...
--
-- **This connection is HELD.** aeon is asked several times per turn -- once to plan a window,
...
-- Where the socket primitive comes from when the caller did not say. Inside aeon the whole library
-- is already there; elsewhere a host that named its own `__stream` is honoured too.
if not transport then
  transport = (_G.aeon and _G.aeon.stream) or _G.__stream
end

local M = { _NAME = "aeon", _VERSION = 1 }
...
local HOSTS = { "aeon", "axon", "hexe", "oslo" }

-- ---------------------------------------------------------------- JSON, in Lua

-- Carried rather than required. The library runs inside somebody else's VM, so it cannot reach
-- aeon's own JSON — and a client that depended on the host having a JSON module would be a client
...
function encode(v, depth)
  depth = (depth or 0) + 1
  if depth > 24 then error("aeon: value nested too deeply to send", 0) end
  ...
  if kind == "number" then
    if v ~= v or v == math.huge or v == -math.huge then
      error("aeon: " .. tostring(v) .. " cannot be sent", 0)
    end
    return (math.type and math.type(v) == "integer") and string.format("%d", v) or tostring(v)
  end
  if kind ~= "table" then error("aeon: cannot send a " .. kind, 0) end
  ...
  for key, value in pairs(v) do
    if type(key) ~= "string" then error("aeon: only string keys can be sent", 0) end
...
local function decode(s)
  ...
  local function fail(why)
    error("aeon: bad reply at " .. at .. ": " .. why, 0)
  end
...
local function exactly(handle, n)
  ...
  while have < n do
    ...
    if #chunk == 0 then return nil, "aeon closed the connection" end
    parts[#parts + 1] = chunk
    have = have + #chunk
  end
...
---
--- The handle is held. aeon serves many requests per connection, so a client that reconnected
...
function Session:call(name, ...)
  ...
  if not reply.ok then return nil, reply.error or "aeon refused the call" end
...
---
--- `$AXON_API_SOCKET` first, because a program aeon started inherits it and means *that* session —
...
--- looks for any of the family's globals rather than only its own. Inside oslo `_G.aeon` does not
...
--- The directory aeon binds its control sockets in.
---
--- Mirrors aeon's own socket directory: the default session binds straight under `<runtime>/aeon`,
...
local function socket_dir()
  ...
  local base = (runtime and runtime ~= "")
    and (runtime .. "/aeon")
    or ("/tmp/aeon-" .. (os.getenv("UID") or "0"))
  local instance = os.getenv("AEON_INSTANCE")
...
--- Open a connection to a running aeon.
---
--- `where` is nothing (find it), a session name, or `{ path = "…", timeout_ms = 5000 }`.
function M.connect(where)
  if not transport then
    return nil, "no transport: pass one to the chunk, as load(src)(aeon.stream)"
  end
  local candidates = find(where)
  if not candidates or #candidates == 0 then
    return nil, "no aeon socket found - `aeon serve` starts one, or `aeon api` answers without it"
  end
  ...
  -- socket timeout, which says nothing about the cause. Inside aeon, `ctx.*`
  -- already reaches everything this library does, without a socket.
  local self_socket = _G.aeon and _G.aeon.__self_socket
  local last
  for _, candidate in ipairs(candidates) do
    if self_socket and candidate.path == self_socket then
      return nil, "that is this aeon; call it directly in here rather than through a socket"
    end

...
//!
//! ```lua
//! local h = aeon.stream.connect(path, timeout_ms)
//! h:send(bytes)   h:recv(n)   h:close()
//! ```
//!
//! This is also what lets aeon dial *out*: a sibling's own client runs unchanged in this VM,
//! given this table.
...
mod tests {
    ...
    fn connecting_to_nothing_answers_nil_and_a_reason() {
        ...
        engine
            .run(
                r#"
                local handle, why = aeon.stream.connect("/no/such/socket")
                aeon.answered = (handle == nil) and (type(why) == "string")
                "#,
                "probe.lua",
            )
            .expect("run");
    ...
    fn the_primitive_is_named_where_a_siblings_client_looks_for_it() {
        ...
        engine
            .run(
                "aeon.found = type(aeon.stream) == \"table\" and type(aeon.stream.connect) == \"function\"",
                "probe.lua",
            )
            .expect("run");
        engine.harvest();

...
/// The keyed registrars the `aeon` module offers.
///
...
pub const SPECS: &str = "__aeon_specs";

/// Registrars a project's own `.aeon.lua` may not use.
///
...
impl Engine {
    /// A VM with the `aeon` module installed and nothing declared.
    #[must_use]
    pub fn new() -> Self {
        let mut engine = Self {
            lua: Lua::full(),
    ...
    pub fn harvest(&mut self) {
        ...
        self.lua.enter(|ctx| {
            let Value::Table(aeon) = ctx.get_global_value("aeon") else {
                return;
            };
            let mut held = config.borrow_mut();
            for (key, value) in aeon.iter(ctx) {
    ...
    ///
    /// `aeon.mask["shell"] = function(item) ... end`. Keyed rather than a list, because only
    ...
    /// against a name aeon does not ask.
    #[must_use]
    ...
    /// Install the `aeon` global.
    fn install(&mut self) {
        let config = Rc::clone(&self.config);
        let logged = Rc::clone(&self.logged);
        self.lua.enter(|ctx| {
            let aeon = Table::new(&ctx);
            ...
            for registrar in REGISTRARS {
                ...
                let callback = Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
                    ...
                    let Value::String(id) = id else {
                        return Err(convert::raise(
                            ctx,
                            &format!("aeon.{name}: the first argument must be a name"),
                        ));
                    };
                    ...
                    let Some(value) = convert::to_json(ctx, spec, 0) else {
                        return Err(convert::raise(
                            ctx,
                            &format!("aeon.{name}({id}): this table cannot be described"),
                        ));
                    };
                ...
                aeon.set(ctx, *registrar, callback).ok();
            }

            {
                ...
                let load = Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
                    ...
                    let Value::String(path) = path else {
                        return Err(convert::raise(ctx, "aeon.load: expects a path"));
                    };
                ...
                aeon.set(ctx, "load", load).ok();
            }

            // Made here so `aeon.decay.normal = 0.02` works without a config writing
            // `aeon.decay = {}` first. Plain settings tables: nothing registers into them, and
            // a config may still replace either wholesale.
            for nested in ["decay", "witness", "buffer", "weights", "budget", "mask"] {
                aeon.set(ctx, nested, Table::new(&ctx)).ok();
            }

            // The socket primitive, so the family's clients run unchanged in this VM. Named
            // twice: `aeon.stream` for a client that knows this host, `__stream` for one that
            // does not.
            let stream = crate::stream::table(ctx);
            aeon.set(ctx, "stream", stream).ok();
            ctx.set_global("__stream", stream);

            ctx.set_global(SPECS, Table::new(&ctx));
            handler::install(ctx, aeon);
            helpers::install(ctx, aeon, &logged);
            ctx.set_global("aeon", aeon);
        });
    }
    ...
    /// `aeon.load` is run straight after it, before the next file, so a config reads top to
        ...
        for registrar in PRIVILEGED {
            ...
            if let Some(first) = added.first() {
                return Err(LuaError::Untrusted {
                    file: path.display().to_string(),
                    what: format!("aeon.{registrar}(\"{first}\")"),
                });
            }
        }

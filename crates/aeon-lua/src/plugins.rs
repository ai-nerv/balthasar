...
pub struct Roots {
    /// `$XDG_CONFIG_HOME/aeon`, the owner's own.
    pub config: Option<PathBuf>,
    /// `$XDG_DATA_HOME/aeon/site`, where installed packages live.
    pub site: Option<PathBuf>,
    /// The working directory, whose `.aeon.lua` may choose but not declare.
...
impl Roots {
    ...
    pub fn discovered(cwd: &Path) -> Self {
        Self {
            config: Some(PathBuf::from(crate::helpers::config_home()).join("aeon")),
            site: Some(PathBuf::from(crate::helpers::data_home()).join("aeon/site")),
...
///   ./.aeon.lua                       may choose, may not declare
/// ```
#[must_use]
pub fn runtimepath(roots: &Roots) -> Vec<(PathBuf, bool)> {
    ...
    if let Some(project) = &roots.project {
        let local = project.join(".aeon.lua");
        if local.is_file() {
...
///
/// `aeon.trusted = { "/home/you/work" }` in the owner's own configuration. A directory under a
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let at = std::env::temp_dir().join(format!("aeon-rtp-{name}"));

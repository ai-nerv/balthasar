...
//! aeon's process, so it gets the handful of things its handlers actually need and not a
//! standard library.
...
/// Anything the config passed to `aeon.log`.
pub type Logged = Rc<RefCell<Vec<String>>>;

/// Install `aeon.json`, `aeon.path`, `aeon.git`, `aeon.text`, `aeon.log` and the few constants.
pub fn install<'gc>(ctx: luna::Context<'gc>, aeon: Table<'gc>, logged: &Logged) {
    aeon.set(ctx, "json", json_table(ctx)).ok();
    aeon.set(ctx, "path", path_table(ctx)).ok();
    aeon.set(ctx, "git", git_table(ctx)).ok();
    aeon.set(ctx, "text", text_table(ctx)).ok();
    aeon.set(ctx, "fs", fs_table(ctx)).ok();
    ...
    aeon.set(ctx, "log", log).ok();

    aeon.set(ctx, "looks_like_secret", secret_check(ctx)).ok();
    ...
    for (name, value) in [
        ...
        aeon.set(ctx, name, text).ok();
    }
}

/// `aeon.json.encode` and `aeon.json.decode`, so a source adapter does not carry a parser.
fn json_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let encode = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Some(json) = convert::to_json(ctx, value, 0) else {
            return Err(convert::raise(
                ctx,
                "aeon.json.encode: cannot be written down",
            ));
        };
    ...
    let decode = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.json.decode: expects a string"));
        };
...
/// `aeon.path.basename` and `aeon.path.join`.
fn path_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let basename = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.path.basename: expects a path"));
        };
...
/// `aeon.git.common_dir`, so a scope handler can put every worktree in one memory.
fn git_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let common = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.git.common_dir: expects a path"));
        };
        let path = String::from_utf8_lossy(text.as_bytes()).into_owned();
...
/// `aeon.text.tail`, for a mask handler trimming a wall of output to its last few lines.
fn text_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let tail = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.text.tail: expects a string"));
        };
...
/// `aeon.fs.glob`, `aeon.fs.read` and `aeon.fs.ls`.
///
...
fn fs_table<'gc>(ctx: luna::Context<'gc>) -> Table<'gc> {
    ...
    let glob = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.fs.glob: expects a pattern"));
        };
    ...
    let read = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        let value: Value = stack.consume(ctx)?;
        let Value::String(text) = value else {
            return Err(convert::raise(ctx, "aeon.fs.read: expects a path"));
        };
        let path = String::from_utf8_lossy(text.as_bytes()).into_owned();
...
/// `aeon.looks_like_secret`, so the shipped gate does not need a regex engine in Lua.
...
mod tests {
    ...
    fn a_glob_matches_by_prefix_and_extension() {
        let dir = std::env::temp_dir().join("aeon-glob-test");

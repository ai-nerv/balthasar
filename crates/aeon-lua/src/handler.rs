...
/// Questions `aeon.on.<name>` may be registered against.
///
...
pub const ON: &str = "__aeon_on";
/// Where told-handlers live inside the VM.
pub const DID: &str = "__aeon_did";
/// Where a call's arguments are left for the generated chunk.
pub const ARGS: &str = "__aeon_args";
/// Where a call's answer is left for Rust.
pub const ANSWER: &str = "__aeon_answer";

/// Build `aeon.on` and `aeon.did`, and the tables behind them.
pub fn install<'gc>(ctx: luna::Context<'gc>, aeon: Table<'gc>) {
    for (field, store, names) in [("on", ON, ASKED), ("did", DID, TOLD)] {
            ...
            let register = Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
                ...
                if !matches!(handler, Value::Function(_)) {
                    return Err(convert::raise(
                        ctx,
                        &format!("aeon.{field}.{which}: expects a function"),
                    ));
                }
        ...
        aeon.set(ctx, field, namespace).ok();
    }
}

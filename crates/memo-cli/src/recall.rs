//! `memo recall` — search, and say why each answer is here.

use crate::Which;
use crate::{now, open, render};
use clap::Parser;
use memo_lua::Floors;
use memo_model::{ScopeId, Tier};
use memo_store::{Recall, Scored, Store};
use std::path::Path;
...
pub fn run(
    ...
    let tiers = match &args.tier {
        Some(name) => vec![
            name.parse::<Tier>()
                .map_err(|_| anyhow::anyhow!("'{name}' is not a tier memo knows"))?,
        ],
        None => Vec::new(),
    };
    ...
    // about the memory.
    let mut from: std::collections::HashMap<String, memo_store::Tool> =
    ...
    let shape = memo_recall::shape_of(&ask.query);
    let policy = if args.lexical_only {
        memo_recall::Policy::lexical_only()
    } else {
        let held = memo_recall::Policy::for_shape(shape);
    ...
    let mut paths: std::collections::HashMap<String, memo_model::Relation> =
        std::collections::HashMap::new();
    if !policy.families.is_empty() {
        let seeds: Vec<memo_model::MemoryId> =
    ...
    let spans_tools = {
        let mut seen: Vec<&memo_store::Tool> = found
            .iter()
    ...
    for hit in &found {
        ...
        if args.explain && !hit.memory.witnesses.is_empty() {
            let ceiling = hit
                ...
                .min()
                .unwrap_or(memo_model::Presentation::Asserted);
            let sources: std::collections::BTreeSet<String> = hit
                .memory
                .witnesses
                .iter()
                .map(memo_model::Witness::domain_of)
                .collect();
            let runs: std::collections::BTreeSet<&str> = hit
                ...
                .collect();
            if ceiling != memo_model::Presentation::Asserted || sources.len() < runs.len() {
                crate::say!(
                    "     {}",
                    render::dim(&format!(
...
fn capture(store: &mut Store, said: &Capturing<'_>) -> anyhow::Result<String> {
    ...
    let id = format!("recall-{at}-{}", &memo_model::content_hash(&ask.query)[..8]);
    let candidates: Vec<memo_store::Candidate> = found
        .iter()
        .enumerate()
    ...
    store.note_recall(
        &memo_store::RecallRun {
            ...
            scope: (*scope).clone(),
            session: None,
            query_hash: memo_model::content_hash(&ask.query)[..16].to_owned(),
            requested_at: at,
...
fn reached(
    store: &Store,
    seeds: &[memo_model::MemoryId],
    policy: &memo_recall::Policy,
    ask: &Recall,
    at: memo_model::Timestamp,
) -> anyhow::Result<Vec<(Scored, memo_model::Relation)>> {
    if policy.families.is_empty() || seeds.is_empty() {
        return Ok(Vec::new());
    }
    let reach = memo_store::Reach {
        hops: policy.hops,
        ..memo_store::Reach::default()
    };
...
fn stores(
    ...
    scope: &ScopeId,
    tool: &Which,
) -> anyhow::Result<Vec<(Store, bool, memo_store::Tool)>> {
    ...
    for named in searched(scope, tool) {
        ...
        if memo_store::scope_path(&everywhere, &named).is_file() {
...
fn searched(scope: &ScopeId, tool: &Which) -> Vec<memo_store::Tool> {
    if tool.named {
        return vec![tool.tool.clone()];
    }
    let present = memo_store::tools_in(scope);
    if present.is_empty() {
        vec![tool.tool.clone()]
    } else {
        present
...
fn shadow(
    ...
    served: &[Scored],
    at: memo_model::Timestamp,
) -> anyhow::Result<Option<memo_recall::Shadow>> {
    let policy = match name {
        "lexical-only" => memo_recall::Policy::lexical_only(),
        "balanced" => memo_recall::Policy::balanced(),
        other => {
            ...
            let shape = match other {
                "temporal" => memo_recall::Shape::Temporal,
                "repair" | "causal" => memo_recall::Shape::Causal,
                "entity" => memo_recall::Shape::Entity,
                "similar" | "semantic" => memo_recall::Shape::Semantic,
                "current-fact" | "current" => memo_recall::Shape::Current,
                "procedure" | "procedural" => memo_recall::Shape::Procedural,
                _ => anyhow::bail!(
                    "no policy called '{other}' — try balanced, lexical-only, temporal, \
                     repair, entity, similar, current-fact or procedure"
                ),
            };
            memo_recall::Policy::for_shape(shape)
        }
    };
    ...
    if !policy.families.is_empty() {
        let seeds: Vec<memo_model::MemoryId> = candidates
            .iter()
            .take(5)
    ...
    Ok(Some(memo_recall::Shadow::of(
        &memo_recall::Policy::balanced(),
        &policy,
        &served_ids,
        &shadow_ids,
        tokens,
...
mod tests {
    use super::*;
    use memo_model::{Body, Memory, Tier as T};

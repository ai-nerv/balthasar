//! `memo export` and `memo import` — the backup, and the way back.
//!
...
pub fn import(
    ...
    for (number, line) in source.lines().enumerate() {
        ...
        match store.remember(memory, witness, at)? {
            memo_store::Landing::Added(_) => added += 1,
            memo_store::Landing::Reinforced(_) => reinforced += 1,
            memo_store::Landing::Superseded { .. } => superseded += 1,
        }
    }

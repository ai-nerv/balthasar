//! `memo replay` — everything a run said, back out again.
//!
...
//! whatever shape the harness uses. memo stores those records and never parses them.
...
pub fn run(
    ...
    for turn in &turns {
        if args.raw {
            // Exactly what the harness wrote. A turn memo was given no record for is skipped

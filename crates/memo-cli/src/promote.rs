...
//!
//! It is the strongest evidence there is, because it is somebody choosing rather than memo
//! inferring. So it crosses alone and it pins.

use crate::Which;
use crate::{now, open, render};
use clap::Parser;
use memo_model::{ScopeId, SessionId, Tier, Witness, WitnessId, WitnessKind};
use std::path::Path;
...
pub fn run(
    ...
    crate::say!(
        "{}",
        render::dim("  memo promote <handle>          keep one of them")
    );
    Ok(())
}
...
fn keep(
    ...
    // than memo inferring, so it crosses alone and it pins.

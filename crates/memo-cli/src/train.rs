//! `memo train` — fit the ranking policy, and say whether to believe it.
//!
...
#[must_use]
pub fn model_path(scope: &ScopeId, tool: &memo_store::Tool) -> PathBuf {
    memo_store::scope_path(scope, tool).with_extension("policy.json")
}
...
pub fn run(
    ...
    let examples = memo_testkit::label(&rows);

    let fitted = match memo_testkit::fit(&examples, args.holdout, args.passes) {
        Ok(held) => held,
        Err(why) => {
    ...
    crate::say!(
        "{}",
        render::dim("it will not be used until you opt in — see `memo recall --shadow`")
    );
    Ok(())
}

/// What it learned, and whether that is worth anything.
fn say(fitted: &memo_testkit::Fitted) {
    crate::say!(
        "{} {}",
...
/// Rows from a file `memo dataset` wrote.
fn read_rows(path: &Path) -> anyhow::Result<Vec<memo_store::TrainingRow>> {

//! `memo` with nothing after it — what this scope remembers.
...
pub fn run(
    ...
    if census.is_empty() {
        ...
        crate::say!(
            "{}",
            render::dim("  memo remember \"we run the tests with make test\"")
        );
        return Ok(());
    }
    ...
    let asserted = store
        .recall(&memo_store::Recall {
            query: String::new(),
            limit: usize::MAX,
            tiers: vec![memo_model::Tier::Fact, memo_model::Tier::Habit],
            floor: floors.inject,

//! What `memo` does when you type it.
//!
...
/// Memory for agents.
#[derive(Debug, Parser)]
#[command(name = "memo", version, about, disable_help_subcommand = true)]
struct Cli {
    ...
    ///
    /// memo keeps one store per tool per project, so a harness remembering a decision and a
    ...
    /// working out whether a problem is memo's or a config's.
    #[arg(long, global = true)]
    no_config: bool,

    /// Treat this unix time as now.
    ///
    /// Everything memo decides is a function of when it is asked — what has faded, what is
...
enum What {
    ...
    Init(init::Args),
    /// Install the shipped configuration into `$XDG_CONFIG_HOME/memo`.
    ...
    Eval(eval::Args),
    /// Print the client library another program loads to talk to memo.
...
fn scope_of(cli: &Cli, loaded: &mut Loaded, cwd: &Path) -> memo_model::ScopeId {
    match cli.scope.as_str() {
        "global" => memo_model::ScopeId::global(),
...
pub(crate) fn scrollback(
    ...
    tool: &Which,
) -> anyhow::Result<memo_store::Transcript> {
    let path = match override_path {
        ...
        None => {
            home(scope)?;
            memo_store::transcript_path(scope, &tool.tool)
        }
    };
    Ok(memo_store::Transcript::open(&path)?)
}
...
#[must_use]
pub(crate) fn weights_of(settings: &memo_lua::Settings, vectors: bool) -> memo_store::Weights {
    let said = settings.weights();
    let asked = memo_store::Weights {
...
#[must_use]
pub(crate) fn now() -> memo_model::Timestamp {
...
pub(crate) fn open(
    ...
    scope: &memo_model::ScopeId,
    tool: &Which,
) -> anyhow::Result<memo_store::Store> {
    let path =
        override_path.map_or_else(|| memo_store::scope_path(scope, &tool.tool), Path::to_owned);
    if override_path.is_none() {
        home(scope)?;
    }
    Ok(memo_store::Store::open(&path)?)
}
...
pub(crate) fn runs_under(
    ...
    match override_path {
        ...
        None => memo_store::home_of(scope).join(tool.tool.as_str()),
    }
}
...
fn home(scope: &memo_model::ScopeId) -> anyhow::Result<()> {
    if let Some(at) = memo_store::project_home(scope) {
        memo_store::make_home(&at)?;
    }
    Ok(())
}
...
fn tool_of(cli: &Cli, loaded: &Loaded) -> anyhow::Result<Which> {
    ...
    match said {
        None => Ok(Which {
            tool: memo_store::Tool::default(),
            named: false,
        }),
        Some(name) => memo_store::Tool::new(&name)
...
mod tests {
    ...
    #[test]
    fn typing_memo_alone_says_what_is_remembered() {
        let cli = Cli::try_parse_from(["memo"]).expect("parse");

//! `memo trust` — where a memory's evidence came from, and what that permits.
//!
//! The fourth explanation view, and deliberately not part of the other three. `memo why` answers
...
use clap::Parser;
use memo_model::{Channel, Presentation, ScopeId};
...
pub fn run(
    ...
    crate::say!(
        "{}",
        render::dim(
            "`memo why` answers whether this is credible. This answers whether it is safe to place in a context."
        )
    );
    Ok(())
}

/// Trust as `memo why` would want to mention it, in one line.
///
...
/// a trusted channel.
#[must_use]
pub fn one_line(witnesses: &[memo_model::Witness]) -> Option<String> {
    ...
    (sources.len() < sessions.len()).then(|| {
        format!(
            "{} session(s) but only {} source(s) — see `memo trust`",
            sessions.len(),
            sources.len()
        )
    })

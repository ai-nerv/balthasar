//! `memo context` — exactly what a model would be told.
//!
...
use memo_recall::{Ask, Bound, Context, Section};
use std::path::Path;
...
pub fn run(
    ...
    anyhow::ensure!(
        !sections.is_empty(),
        "no sections are declared — `memo configs` installs the ones memo ships"
    );
    ...
    let context = memo_recall::assemble(&stores, &sections, &ask, |text, memory| {

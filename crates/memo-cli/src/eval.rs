...
use crate::render;
use clap::Parser;
use memo_testkit::{Baseline, Scenario, Score};
...
pub fn run(args: &Args) -> anyhow::Result<()> {
    ...
    let with = memo_testkit::run(&scenario, true);
    let without = memo_testkit::run(&scenario, false);
    let ceiling = Score::ceiling(&scenario);

    if let Some(into) = &args.artifact {
        let path = into.join(format!("memo-baseline-{}.json", baseline.run_id));
    ...
    if args.full {
        let held = memo_testkit::Full::measure(&scenario, name, START);
        if args.json {
...
fn say_full(held: &memo_testkit::Full) {
    let b = &held.baseline;
...
fn say_external(at: &std::path::Path) {
    ...
    for family in [
        ...
        let found = memo_testkit::load(family, at);
...
///
/// Not a benchmark of memo against something else — a demonstration that the two numbers are

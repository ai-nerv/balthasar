//! `memo relate` — derive the relationship index, and say what it found.
//!
...
use clap::Parser;
use memo_distil::{Thresholds, entities, overlap, temporal};
use memo_model::{Derivation, ScopeId};
use std::path::Path;
...
pub fn run(
    ...
    if args.rebuild {
        store.retire_relations(Derivation::Rule, memo_distil::RELATION_DERIVATION, at)?;
        store.retire_relations(Derivation::Structure, memo_distil::RELATION_DERIVATION, at)?;
    }
    ...
    let named: Vec<(memo_model::MemoryId, Vec<String>)> = held
        .iter()
        .map(|m| {
            let names = memo_store::entities_in(&m.text())
                .into_iter()
                .map(|e| e.name)
                .collect();
            (m.id.clone(), names)
    ...
    let rarity = |name: &str| memo_store::rarity(counts.get(name).copied().unwrap_or(0));
    edges.extend(entities(&named, &rarity, &rules, at));

    let texts: Vec<(memo_model::MemoryId, String)> =

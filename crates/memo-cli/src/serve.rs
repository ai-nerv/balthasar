//! `memo serve`, `memo api` and `memo lua-api` — the three ways in from outside.
//!
...
use clap::Parser;
use memo_host::{Answering, Door};
use memo_ipc::{Listener, Peer, Reply, Request};
...
pub struct ServeArgs {
    /// A name, when more than one memo should be reachable at once.
...
pub fn serve(
    ...
    let descriptor = memo_ipc::tool_descriptor()?;
    ...
    let mut opened: std::collections::HashMap<memo_store::Tool, Opened> =
    ...
    listener.serve(|peer: &Peer, request: Request| {
            ...
            std::collections::hash_map::Entry::Vacant(seat) => {
                ...
                let made = open(store_path, scope, &which).and_then(|store| {
                    crate::scrollback(store_path, scope, &which).map(|scrollback| Opened {
                        store,
                        scrollback,
                        scratch: memo_store::Scratchpad::at(runs_under(store_path, scope, &which)),
                    })
                });
                match made {
                    Ok(ready) => {
        ...
        memo_host::answer_with(&mut at, &Door::Socket(peer.clone()), &request, |entry| {
            loaded.mask(entry)
        })
    })?;
    Ok(())
...
pub fn api(
    ...
    let reply = match open(store_path, scope, tool).and_then(|store| {
        ...
        Ok((mut store, mut scrollback)) => {
            ...
            memo_host::answer_with(&mut at, &Door::Owner, &request, |entry| loaded.mask(entry))
        }
...
fn named_by_kernel(peer: &Peer) -> Option<memo_store::Tool> {
    peer.program
        .as_deref()
        .and_then(|program| {
            Path::new(program)
        ...
        .and_then(|name| memo_store::Tool::from_program(&name))
}

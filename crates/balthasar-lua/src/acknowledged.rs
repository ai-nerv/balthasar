...
//! acknowledged it; a file under `site/pack/` whose digest does not match does not run. A file in
//! your own `plugin/` directory is one you put there and needs no acknowledgement.
...
/// The digest of a file's contents, as the manifest records it: of the bytes, not of the path.
#[must_use]
...
///
/// A manifest that will not parse reads as empty, which holds everything back.
#[must_use]
...
mod tests {
    ...
    fn a_file_that_changed_after_being_acknowledged_is_not_cleared_and_is_known() {
        let dir = Scratch::new("balthasar-ack", "changed");
    ...
    fn acknowledging_replaces_rather_than_accumulates() {
        let dir = Scratch::new("balthasar-ack", "replace");
        let manifest = manifest_in(&dir);
    ...
    fn a_manifest_that_will_not_parse_holds_everything_back() {
        let dir = Scratch::new("balthasar-ack", "broken");
        let manifest = manifest_in(&dir);

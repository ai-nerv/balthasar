...
//! ships `hexe.lua`, and this is the same file with balthasar's identity and balthasar's verbs. The
...
mod tests {
    ...
    fn the_stub_carries_its_own_json() {
        // It runs inside somebody else's VM, so it cannot reach balthasar's. A client that needed
    ...
    fn the_stub_holds_its_connection() {
        // balthasar is asked several times per turn, which is not how a control socket that gets
    ...
    fn the_stub_looks_for_every_sibling_rather_than_only_itself() {
        ...
        // business, and the stub is Lua. Naming one here would put a harness in balthasar's Rust,

...
use aeon_model::{Memory, Timestamp, Witness};
use std::io::Write;

/// Write one line to standard output, and stop quietly when nobody is reading.
///
/// `println!` panics on a broken pipe, so `aeon recall | head` ended in a backtrace rather than
...
/// Otherwise: only when stdout is a terminal, so `aeon recall | grep` sees plain text.
#[must_use]
pub fn styled() -> bool {
...
pub fn standing(memory: &Memory, inject_floor: f64, now: Timestamp) -> &'static str {
    if memory.archived_at.is_some() {
        ...
        && memory.strength.at(now) >= aeon_model::floor::SPENT
    {
        "asserted"
    } else {
        "findable"
...
pub fn withheld(memory: &Memory, inject_floor: f64, now: Timestamp) -> Option<String> {
    ...
    if memory.strength.at(now) < aeon_model::floor::SPENT {
        return Some(format!(
...
pub fn origin(memory: &Memory, project: Option<&str>, session: Option<&str>) -> String {
    ...
    match session.or(memory.session.as_ref().map(aeon_model::SessionId::as_str)) {
...
/// so two memories written in the same moment share them — which had `aeon recall` printing
/// two different facts under one handle and `aeon why` refusing both as ambiguous. The trailing
...
/// One witness, as `aeon why` prints it.
///
...
mod tests {
    ...
    fn a_memory_says_which_project_it_belongs_to() {
        use aeon_model::{Body, MemoryId, ScopeId, Tier};
        ...
        m.session = Some(aeon_model::SessionId::new("01M1CTNN7SZ613D58ZXM4JYT8Z"));
        assert!(

...
//!
//! The preview is not a convenience. Forgetting is the one thing aeon does that a person
...
fn say(report: &Faded) {
    ...
    if !report.swept.is_empty() {
        ...
        crate::say!(
            "  {}",
            render::dim(
                "kept in full, searched with --archived, and still explained by `aeon why`"
            )
        );
    }
    ...
    if report.preview {
        ...
        crate::say!(
            "{}",
            render::dim("nothing was written — `aeon decay --now` to apply")
        );
    }
}
...
fn as_json(report: &Faded) -> String {
    let entries = |list: &[aeon_store::Weakened]| {
        list.iter()
            .map(|e| {
                serde_json::json!({
                    "id": e.id.to_string(),

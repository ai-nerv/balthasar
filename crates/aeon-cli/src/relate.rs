//! `aeon relate` — derive the relationship index, and say what it found.
//!
...
use aeon_distil::{Thresholds, entities, overlap, temporal};
use aeon_model::{Derivation, ScopeId};
use clap::Parser;
use std::path::Path;
...
pub fn run(
    ...
    if args.rebuild {
        store.retire_relations(Derivation::Rule, aeon_distil::RELATION_DERIVATION, at)?;
        store.retire_relations(Derivation::Structure, aeon_distil::RELATION_DERIVATION, at)?;
    }
    ...
    let named: Vec<(aeon_model::MemoryId, Vec<String>)> = held
        .iter()
        .map(|m| {
            let names = aeon_store::entities_in(&m.text())
                .into_iter()
                .map(|e| e.name)
                .collect();
            (m.id.clone(), names)
    ...
    let rarity = |name: &str| aeon_store::rarity(counts.get(name).copied().unwrap_or(0));
    edges.extend(entities(&named, &rarity, &rules, at));

    let texts: Vec<(aeon_model::MemoryId, String)> =

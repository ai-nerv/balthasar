//! `aeon configs` — put the shipped configuration where aeon reads it.
//!
...
// The files aeon ships, compiled in so a binary is enough on its own.
//
...
pub fn run(args: &Args) -> anyhow::Result<()> {
    let into = PathBuf::from(config_home()).join("aeon");

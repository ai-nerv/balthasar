//! `aeon remember` — the manual door onto the ladder.
//!
...
pub fn run(
    ...
    let body = match (&args.subject, &args.predicate) {
        ...
        // model, and aeon requires neither in order to work.
...
fn announce(
    ...
    let described = |id: &aeon_model::MemoryId| -> anyhow::Result<serde_json::Value> {
    ...
    match landing {
        ...
        Landing::Superseded { was, .. } => {
            ...
            crate::say!(
                "replaced {}",
                old.as_ref()
                    .map_or_else(|| render::short(&was.to_string()), aeon_model::Memory::text)
            );
            crate::say!(
                "     {}",

//! `aeon trust` — where a memory's evidence came from, and what that permits.
//!
//! The fourth explanation view, and deliberately not part of the other three. `aeon why` answers
...
use aeon_model::{Channel, Presentation, ScopeId};
use clap::Parser;
...
pub fn run(
    ...
    crate::say!(
        "{}",
        render::dim(
            "`aeon why` answers whether this is credible. This answers whether it is safe to place in a context."
        )
    );
    Ok(())
}

/// Trust as `aeon why` would want to mention it, in one line.
///
...
/// a trusted channel.
#[must_use]
pub fn one_line(witnesses: &[aeon_model::Witness]) -> Option<String> {
    ...
    (sources.len() < sessions.len()).then(|| {
        format!(
            "{} session(s) but only {} source(s) — see `aeon trust`",
            sessions.len(),
            sources.len()
        )
    })

...
use aeon_lua::{Engine, LuaError, Roots, Settings};
use aeon_model::ScopeId;
use std::path::Path;

/// What a configuration said about embedding.
fn embedder_from(config: &aeon_lua::Config) -> Option<Box<dyn aeon_embed::Embed>> {
    let said = config.get("embedder").map(|value| aeon_embed::Value {
        kind: value
            .get("kind")
    ...
    aeon_embed::open(&aeon_embed::Spec::read(said.as_ref()))
}
...
pub struct Loaded {
    engine: Engine,
    settings: Settings,
    embedder: Option<Box<dyn aeon_embed::Embed>>,
}
...
impl Loaded {
    ...
    pub fn read(cwd: &Path) -> Result<Self, LuaError> {
        ...
        let mut files = aeon_lua::runtimepath(&roots);

        // Trust is decided after the owner's own files have run, because `aeon.trusted` is one
        ...
        for (path, allowed) in &mut files {
            *allowed = path
                .parent()
                .is_some_and(|dir| aeon_lua::vouched_for(&trusted, dir));
        }
        engine.read(&files)?;
    ...
    pub fn bare() -> Self {
        Self {
            ...
            embedder: aeon_embed::open(&aeon_embed::Spec::default()),
        }
    }

    /// The embedder, if one is configured and available.
    #[must_use]
    pub fn embedder(&self) -> Option<&dyn aeon_embed::Embed> {
    ...
    #[must_use]
    pub fn distillers(&self) -> (Vec<Box<dyn aeon_distil::Distil>>, Vec<String>) {
        aeon_distil::backends(self.engine.config().get("distiller"))
    }
    ...
    pub fn ingest(
        &mut self,
        store: &mut aeon_store::Store,
        settings: &Settings,
        ask: &aeon_distil::Ingest,
    ) -> Result<aeon_distil::Report, aeon_distil::DistilError> {
        aeon_distil::ingest(&mut self.engine, store, settings, ask)
    }
    ...
    #[must_use]
    pub fn config(&self) -> aeon_lua::Config {
        self.engine.config()
    }
    ...
    pub fn mask(&mut self, entry: &aeon_store::Entry) -> Option<String> {

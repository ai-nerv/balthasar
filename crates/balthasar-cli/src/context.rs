...
use balthasar_recall::{Ask, Bound, Context, Section};
use clap::Parser;
use std::path::Path;
...
pub fn run(
    ...
    anyhow::ensure!(
        !sections.is_empty(),
        "no sections are declared — `balthasar configs` installs the ones balthasar ships"
    );
    ...
    let context = balthasar_recall::assemble(&stores, &sections, &ask, |text, memory| {

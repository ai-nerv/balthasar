//! `balthasar recall` — search, and say why each answer is here.

use crate::Which;
use crate::{now, open, render};
use balthasar_lua::Floors;
use balthasar_model::{ScopeId, Tier};
use balthasar_store::{Recall, Scored, Store};
use clap::Parser;
use std::path::Path;
...
pub fn run(
    ...
    let tiers = match &args.tier {
        Some(name) => vec![
            name.parse::<Tier>()
                .map_err(|_| anyhow::anyhow!("'{name}' is not a tier balthasar knows"))?,
        ],
        None => Vec::new(),
    };
    ...
    // about the memory.
    let mut from: std::collections::HashMap<String, balthasar_store::Tool> =
    ...
    let shape = balthasar_recall::shape_of(&ask.query);
    let policy = if args.lexical_only {
        balthasar_recall::Policy::lexical_only()
    } else {
        let held = balthasar_recall::Policy::for_shape(shape);
    ...
    let mut paths: std::collections::HashMap<String, balthasar_model::Relation> =
        std::collections::HashMap::new();
    if !policy.families.is_empty() {
        let seeds: Vec<balthasar_model::MemoryId> =
    ...
    let spans_tools = {
        let mut seen: Vec<&balthasar_store::Tool> = found
            .iter()
    ...
    for hit in &found {
        ...
        if args.explain && !hit.memory.witnesses.is_empty() {
            let ceiling = hit
                ...
                .min()
                .unwrap_or(balthasar_model::Presentation::Asserted);
            ...
            let runs: std::collections::BTreeSet<&str> = hit
                ...
                .collect();
            if ceiling != balthasar_model::Presentation::Asserted || sources.len() < runs.len() {
                crate::say!(
                    "     {}",
                    render::dim(&format!(
...
fn capture(store: &mut Store, said: &Capturing<'_>) -> anyhow::Result<String> {
    ...
    let id = format!(
        "recall-{at}-{}",
        &balthasar_model::content_hash(&ask.query)[..8]
    );
    let candidates: Vec<balthasar_store::Candidate> = found
        .iter()
        .enumerate()
    ...
    store.note_recall(
        &balthasar_store::RecallRun {
            ...
            scope: (*scope).clone(),
            session: None,
            query_hash: balthasar_model::content_hash(&ask.query)[..16].to_owned(),
            requested_at: at,
...
fn reached(
    store: &Store,
    seeds: &[balthasar_model::MemoryId],
    policy: &balthasar_recall::Policy,
    ask: &Recall,
    at: balthasar_model::Timestamp,
) -> anyhow::Result<Vec<(Scored, balthasar_model::Relation)>> {
    if policy.families.is_empty() || seeds.is_empty() {
        return Ok(Vec::new());
    }
    let reach = balthasar_store::Reach {
        hops: policy.hops,
        ..balthasar_store::Reach::default()
    };
...
fn stores(
    ...
    scope: &ScopeId,
    tool: &Which,
) -> anyhow::Result<Vec<(Store, bool, balthasar_store::Tool)>> {
    ...
    for named in searched(scope, tool) {
        ...
        if balthasar_store::scope_path(&everywhere, &named).is_file() {
...
fn searched(scope: &ScopeId, tool: &Which) -> Vec<balthasar_store::Tool> {
    if tool.named {
        return vec![tool.tool.clone()];
    }
    let present = balthasar_store::tools_in(scope);
    if present.is_empty() {
        vec![tool.tool.clone()]
    } else {
        present
...
fn shadow(
    ...
    served: &[Scored],
    at: balthasar_model::Timestamp,
) -> anyhow::Result<Option<balthasar_recall::Shadow>> {
    let policy = match name {
        "lexical-only" => balthasar_recall::Policy::lexical_only(),
        "balanced" => balthasar_recall::Policy::balanced(),
        other => {
            ...
            let shape = match other {
                "temporal" => balthasar_recall::Shape::Temporal,
                "repair" | "causal" => balthasar_recall::Shape::Causal,
                "entity" => balthasar_recall::Shape::Entity,
                "similar" | "semantic" => balthasar_recall::Shape::Semantic,
                "current-fact" | "current" => balthasar_recall::Shape::Current,
                "procedure" | "procedural" => balthasar_recall::Shape::Procedural,
                _ => anyhow::bail!(
                    "no policy called '{other}' — try balanced, lexical-only, temporal, \
                     repair, entity, similar, current-fact or procedure"
                ),
            };
            balthasar_recall::Policy::for_shape(shape)
        }
    };
    ...
    if !policy.families.is_empty() {
        let seeds: Vec<balthasar_model::MemoryId> = candidates
            .iter()
            .take(5)
    ...
    Ok(Some(balthasar_recall::Shadow::of(
        &balthasar_recall::Policy::balanced(),
        &policy,
        &served_ids,
        &shadow_ids,
        tokens,
...
mod tests {
    use super::*;
    use balthasar_model::{Body, Memory, Tier as T};
    ...
    fn a_breakdown_names_every_signal_that_moved_the_score() {
        ...
        let memory = Memory::new(
            balthasar_model::MemoryId::new("x"),
            T::Fact,
            ScopeId::global(),
            Body::fact("a", "b", "c"),
            0,

...
use crate::Which;
use crate::{now, open};
use balthasar_model::{Memory, ScopeId};
use clap::Parser;
...
pub fn import(
    ...
    for (number, line) in source.lines().enumerate() {
        ...
        match store.remember(memory, witness, at)? {
            balthasar_store::Landing::Added(_) => added += 1,
            balthasar_store::Landing::Reinforced(_) => reinforced += 1,
            balthasar_store::Landing::Superseded { .. } => superseded += 1,
        }
    }

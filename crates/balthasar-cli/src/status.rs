//! `balthasar` with nothing after it — what this scope remembers.
...
pub fn run(
    ...
    if census.is_empty() {
        ...
        crate::say!(
            "{}",
            render::dim("  balthasar remember \"we run the tests with make test\"")
        );
        return Ok(());
    }
    ...
    let asserted = store
        .recall(&balthasar_store::Recall {
            query: String::new(),
            limit: usize::MAX,
            tiers: vec![balthasar_model::Tier::Fact, balthasar_model::Tier::Habit],
            floor: floors.inject,

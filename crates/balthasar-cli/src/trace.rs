//! `balthasar trace` and `balthasar utility` — the two views the ledger adds.
//!
//! Deliberately separate from `balthasar why`. That one answers *why do you believe this*, from
...
fn say_utility(
    ...
    id: &MemoryId,
    held: &balthasar_model::Utility,
    considered: usize,
    returned: usize,
    _at: balthasar_model::Timestamp,
) -> anyhow::Result<()> {
    ...
    crate::say!(
        "{}",
        render::dim(
            "`balthasar why` answers whether this is true. This answers whether using it helped."
        )
    );
    Ok(())
}
...
///
/// Explicit, and never automatic. balthasar starts no training jobs, accumulates no dataset in the

//! `balthasar ingest` — reading transcripts that already exist.
//!
...
use balthasar_distil::{Ingest, Report};
    ...
    if !declared.contains(&args.source) {
        anyhow::bail!(
            ...
            if declared.is_empty() {
                " — none are declared; `balthasar configs` installs the ones balthasar ships"
                    .to_owned()
            } else {

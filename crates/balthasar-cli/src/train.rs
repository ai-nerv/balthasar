//! `balthasar train` — fit the ranking policy, and say whether to believe it.
//!
...
#[must_use]
pub fn model_path(scope: &ScopeId, tool: &balthasar_store::Tool) -> PathBuf {
    balthasar_store::scope_path(scope, tool).with_extension("policy.json")
}
...
pub fn run(
    ...
    let examples = balthasar_testkit::label(&rows);

    let fitted = match balthasar_testkit::fit(&examples, args.holdout, args.passes) {
        Ok(held) => held,
        Err(why) => {
    ...
    crate::say!(
        "{}",
        render::dim("it will not be used until you opt in — see `balthasar recall --shadow`")
    );
    Ok(())
}

/// What it learned, and whether that is worth anything.
fn say(fitted: &balthasar_testkit::Fitted) {
    crate::say!(
        "{} {}",
...
/// Rows from a file `balthasar dataset` wrote.
fn read_rows(path: &Path) -> anyhow::Result<Vec<balthasar_store::TrainingRow>> {

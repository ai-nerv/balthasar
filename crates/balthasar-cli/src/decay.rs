...
//!
//! The preview is not a convenience. Forgetting is the one thing balthasar does that a person
//! cannot undo by asking again, so it defaults to showing rather than doing: `balthasar decay`
//! rehearses, and `balthasar decay --now` commits.

use crate::Which;
use crate::{now, open, render};
use balthasar_model::ScopeId;
use balthasar_store::Faded;
use clap::Parser;
use std::path::Path;
...
fn say(report: &Faded) {
    ...
    if !report.swept.is_empty() {
        ...
        crate::say!(
            "  {}",
            render::dim(
                "kept in full, searched with --archived, and still explained by `balthasar why`"
            )
        );
    }
    ...
    if report.preview {
        ...
        crate::say!(
            "{}",
            render::dim("nothing was written — `balthasar decay --now` to apply")
        );
    }
}

//! `balthasar distil` — read what the runs here actually said.
//!
//! The extractive rules used to run only on `balthasar ingest`, which reads a *harness's* own journal
//! files. A session streamed straight into balthasar's scrollback was never read by them: it earned
...
pub fn run(
    ...
    let runs = match &args.session {
        ...
        None => balthasar_distil::undistilled(&store, &held, PER_PASS)?,
    };
...
pub(crate) fn pass(
    ...
    dry_run: bool,
    loaded: &mut crate::loaded::Loaded,
) -> anyhow::Result<balthasar_distil::Report> {
    let ask = balthasar_distil::Ingest {
        source: balthasar_distil::TRANSCRIPT_SOURCE.to_owned(),
        scope: scope.clone(),
        since: None,
        dry_run,
        now: at,
    };
    let mut total = balthasar_distil::Report {
        dry_run,
        ..balthasar_distil::Report::default()
    };
...
fn say(report: &balthasar_distil::Report, waiting: usize, explain: bool) {
    ...
    if report.dry_run && report.proposed > 0 {
        ...
        crate::say!(
            "{}",
            render::dim("nothing was written — `balthasar distil --now` to apply")
        );
    }
}

...
#
#   A `SAFETY:` note does not count, from the `SAFETY:` line to the end of its block. An `unsafe`
#   block is required to carry one, the requirement is not this gate's to override, and the note
#   has to say enough to be checked against. Prose above the note counts, so a paragraph cannot
#   be parked next to one to escape the budget.
#

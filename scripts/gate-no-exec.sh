...
# execute it.
#
# aeon stores procedures precise enough to reuse, which is exactly what makes this gate
...
#
# The distiller's shell-out backend is the one place aeon runs anything, and it runs a
# CONFIGURED command against text — never a stored procedure. It is named here rather than
# exempted by silence.
set -eu

ALLOWED='crates/aeon-distil/src/distil.rs'
...
if grep -rn 'format!' crates/aeon-model/src/skill.rs | grep -qi 'command\|exec\|sh -c'; then

| **F6** | built | eight named policies chosen by query shape, degrading rather than swapping when vectors are absent; `--lexical-only` as the permanent control; every decision in `--explain`; bounded shadow comparison |

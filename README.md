...
```sh
aeon remember "we run the tests with make test"
aeon recall "tests" --explain
aeon why <handle>            # the evidence, not just the number
aeon decay                   # what today's forgetting would take, before it takes it
```
...
`aeon why` prints an argument rather than a number:
...
purpose — every one of those is a column. `aeon forget --purge` is the single exception, and it
...
```sh
aeon serve                     # listen for a harness
aeon ingest --source axon      # or read transcripts that already exist
aeon consolidate               # carry what recurred into the project's memory
aeon context "run the tests"   # exactly what a model would be told
...
it. `aeon eval` runs that N times with memory and without. Synthetic and reproducible on
...
The last one is the load-bearing one. A model makes aeon better; its absence never makes aeon

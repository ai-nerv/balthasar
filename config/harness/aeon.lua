-- What a harness adds to use aeon.
--
...
--
--   local memory = require("aeon.harness")     -- or load this file
...
-- only use the first two: from a sibling that already carries it, from `aeon lua-api`, or from
-- the `client` verb on an open connection.

local M = { _NAME = "aeon.harness", _VERSION = 1 }

-- The connection, held. aeon is asked several times per turn -- once to observe, once to plan,
...
--- one more attempt, for a harness that knows aeon has just been started.
function M.open(opts)
  opts = opts or {}
  if held then return held end
  if tried and not opts.retry then return nil, "aeon was not reachable earlier this session" end
  tried = true

  if not opts.source then return nil, "aeon's client library was not installed" end
...
--- make a turn wait. What is lost when aeon is down is one turn's observation, and the harness
...
--- Ask what to send.
---
--- `nil` means aeon had nothing to say, and a harness that gets it should do whatever it did
--- before aeon existed. That is the whole degradation story: not an error to handle, an absence
--- to carry on through.

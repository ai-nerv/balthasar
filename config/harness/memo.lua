-- What a harness adds to use memo.
--
...
--
--   local memory = require("memo.harness")     -- or load this file
...
-- only use the first two: from a sibling that already carries it, from `memo lua-api`, or from
-- the `client` verb on an open connection.

local M = { _NAME = "memo.harness", _VERSION = 1 }

-- The connection, held. memo is asked several times per turn -- once to observe, once to plan,
...
--- one more attempt, for a harness that knows memo has just been started.
function M.open(opts)
  opts = opts or {}
  if held then return held end
  if tried and not opts.retry then return nil, "memo was not reachable earlier this session" end
  tried = true

  if not opts.source then return nil, "memo's client library was not installed" end
...
--- make a turn wait. What is lost when memo is down is one turn's observation, and the harness
...
--- Ask what to send.
---
--- `nil` means memo had nothing to say, and a harness that gets it should do whatever it did
--- before memo existed. That is the whole degradation story: not an error to handle, an absence
--- to carry on through.

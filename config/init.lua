-- aeon's configuration, and its only entry point.
--
...
--
--   aeon.on.<question>   ASKED.  nil = not mine, carry on. A table = do this instead.
--                                First non-nil wins.
--   aeon.did.<verb>      TOLD.   Pure side effect. What it returns is ignored.
--
-- Nothing else here takes a function, so there is no third contract to remember.

-- Source adapters, so `aeon ingest` has something to read. A harness is a file in here, not
-- a release: aeon defines the transcript shape and each of these converts to it.
aeon.load("sources/axon.lua")
aeon.load("sections.lua")
...
--
-- aeon.inject_floor = 0.35
-- aeon.live_floor   = 0.10

-- Ebbinghaus, per importance class. Nested data stays nested: setting one leaves the rest.
--
-- aeon.decay.normal  = 0.05    -- ~14-day half-life
-- aeon.decay.high    = 0.01    -- ~69
-- aeon.decay.inertia = true    -- what is needed often resists fading
...
--
-- aeon.witness.distillation = 0.3
-- aeon.witness.correction   = 0.8

-- Words that mark a user's turn as an instruction to remember.
--
-- aeon.imperatives = { "remember", "always", "never", "from now on", "note that" }
...
--   if cwd:find("^" .. aeon.home .. "/scratch/") then return { id = "global" } end
-- end)
...
aeon.on.promote(function(candidate)
  if aeon.looks_like_secret(candidate.text or "") then
...
-- the vector lands.
--
-- aeon.embedder = { kind = "onnx", model = "bge-small-en-v1.5", dimensions = 384 }

-- ------------------------------------------------------------------ trust

-- Directories whose own `.aeon.lua` may DECLARE as well as choose. A project file may
